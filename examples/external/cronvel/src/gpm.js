...
gpm.connectStructureBuffer = (gpmConnect) => {
    ...
    if (gpmConnect.eventMask === undefined) {
        gpmConnect.eventMask = 65535;
    }
    if (gpmConnect.defaultMask === undefined) {
        gpmConnect.defaultMask = 0;
    }
    if (gpmConnect.minMod === undefined) {
        gpmConnect.minMod = 0;
    }
    if (gpmConnect.maxMod === undefined) {
        gpmConnect.maxMod = 65535;
    }
...
gpm.RGT = 8;
/* GPM event handler */
gpm.Handler = function Handler() {
    throw new Error("[terminal] Cannot create a gpm.Handler object directly, use gpm.createHandler() instead");
};
...
gpm.createHandler = function (options) {
    ...
    // Re-emit event
    handler.socket.on("error", (error) => {
        handler.emit("error", error);
        handler.close();
    });
    handler.socket.on("end", () => {
        handler.emit("end");
    });
    handler.socket.on("close", () => {
        handler.emit("close");
        handler.close();
    });
