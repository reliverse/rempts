...
*/
export default function singleColumnMenu(menuItemsArg, options, callback) {
      if (arguments.length < 1) {
        throw new Error(
          "[terminal] singleColumnMenu() needs at least an array of menuItems",
        );
      }
      if (!Array.isArray(menuItemsArg) || !menuItemsArg.length) {
        throw new TypeError(
          "[terminal] singleColumnMenu(): argument #0 should be a non-empty array",
        );
      }
      ...
      if (options.leftPadding === undefined) {
        options.leftPadding = " ";
      }
      if (options.selectedLeftPadding === undefined) {
        options.selectedLeftPadding = options.leftPadding;
      }
      if (options.submittedLeftPadding === undefined) {
      ...
      var computeItems = (menuItems_) => {
        ...
        paddingLength = Math.max(
          stringWidth(options.leftPadding),
          stringWidth(options.selectedLeftPadding),
        );
        ...
        if (
          !options.oneLineItem &&
          textWidth > options.itemMaxWidth - paddingLength
        ) {
      ...
      var prepareArea = (x, y) => {
        ...
        scrollLines =
          start.y +
          lineCount -
          (options.scrollRegionBottom || this.height) -
          1 +
          options.extraLines;
        ...
        item.displayText.forEach((text, line) => {
          ...
          if (paused || options.unsubmittableIndexes[index]) {
            if (index === submittedIndex) {
              if (line) {
                options.disabledSubmittedStyle.forceStyleOnReset.noFormat(
                  options.leftPadding,
                );
              } else {
                options.disabledSubmittedStyle.forceStyleOnReset.noFormat(
                  options.submittedLeftPadding,
                );
              }
              ...
              if (line) {
                options.disabledSelectedStyle.forceStyleOnReset.noFormat(
                  options.leftPadding,
                );
              } else {
                options.disabledSelectedStyle.forceStyleOnReset.noFormat(
            ...
            if (line) {
              options.submittedStyle.forceStyleOnReset.noFormat(
                options.leftPadding,
              );
            } else {
              options.submittedStyle.forceStyleOnReset.noFormat(
            ...
            if (line) {
              ...
              options.selectedStyle.forceStyleOnReset.noFormat(
                options.selectedLeftPadding,
              );
            }
      ...
      var submit = () => {
        if (
          submittedIndex !== null ||
          options.unsubmittableIndexes[selectedIndex]
        ) {
          return;
        }
      ...
      var onMouse = (name, data) => {
        ...
        for (i = 0; i < menuItems.length; i++) {
          ...
          yMax =
            start.y + menuItems[i].offsetY + menuItems[i].displayText.length - 1;
