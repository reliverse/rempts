...
ScreenBufferHD.createFromString = function (options, data) {
  ...
  if (typeof data !== "string") {
    if (!data.toString) {
      throw new Error(
        "[terminal] ScreenBufferHD.createFromDataString(): argument #1 should be a string or provide a .toString() method.",
      );
    }
    data = data.toString();
  }
...
var colorScheme = require("./colorScheme/gnome.json").map((o) => ({
...
function attrEquals(attr1, attr2) {
  ...
  if (
    (attr1[BPOS_MISC] & MISC_ATTR_MASK) !==
...
ScreenBufferHD.loadImage = termkit.image.load.bind(
  ScreenBufferHD,
  ScreenBufferHD.fromNdarrayImage,
);
...
ScreenBufferHD.prototype.writeAttr = function (
  buffer,
  attr,
  at,
  fullWidth = 0,
...
ScreenBufferHD.prototype.generateDeltaEscapeSequence = function (
  term,
  attr,
  lastAttr,
) {
  ...
  if (
    ...
    esc += term.optimized.bgColor24bits(
      attr[BPOS_BG_R],
      attr[BPOS_BG_G],
      attr[BPOS_BG_B],
    );
...
ScreenBufferHD.prototype.CLEAR_BUFFER = Buffer.allocUnsafe(
  ScreenBufferHD.prototype.ITEM_SIZE,
);
ScreenBufferHD.prototype.CLEAR_ATTR.copy(ScreenBufferHD.prototype.CLEAR_BUFFER);
ScreenBufferHD.prototype.CLEAR_BUFFER.write(
  " \x00\x00\x00",
  ScreenBufferHD.prototype.ATTR_SIZE,
); // space
...
ScreenBufferHD.loadSyncV2 = function (filepath) {
  ...
  try {
    ...
    throw new Error(
      "No correct one-lined JSON header found: this is not a ScreenBufferHD file",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Missing mandatory header data, this is a corrupted or obsolete ScreenBufferHD file",
    );
  }
