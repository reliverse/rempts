...
export const colorNameToIndex = (color) => colorNameToIndexDict[color.toLowerCase()];
export const indexToColorName = (index) => indexToColorNameArray[index];
...
export const color2index = exports.colorNameToIndex;
export const index2color = exports.indexToColorName;
export const hexToColor = exports.hexToRgba;
export const stripControlChars = (str, newline) => {
    if (newline) {
        return str.replace(/[\x00-\x09\x0b-\x1f\x7f]/g, "");
    }
    return str.replace(/[\x00-\x1f\x7f]/g, "");
};
export const stripEscapeSequences = (str) => str.replace(escapeSequenceRegex, "");
...
export const truncateString = (str, maxWidth) => {
    var matches, width = 0;
    lastTruncateWidth = 0;
    // Reset
    escapeSequenceParserRegex.lastIndex = 0;
        ...
        if (matches[2]) {
            ...
            if (width >= maxWidth) {
                ...
                return (str.slice(0, matches.index) +
                    string.unicode.truncateWidth(matches[2], maxWidth - lastTruncateWidth));
            }
...
export const truncateMarkupString = (str, maxWidth) => {
...
export const escapeSequenceSkipFn = (strArray, index) => {
    //console.error( '>>> Entering' ) ;
    var code;
    if (strArray[index] !== "\x1b") {
        return index;
    }
    index++;
    if (strArray[index] !== "[") {
...
export const wordWrapAnsi = (str, width) => string.wordwrap(str, {
    width: width,
    noJoin: true,
    fill: true,
    regroupFn: (strArray) => {
...
export const wordWrapMarkup = (str, width) => string.wordwrap(str, {
    width: width,
    noJoin: true,
    fill: true,
    regroupFn: (strArray) => {
...
export const preserveMarkupFormat = string.createFormatter({
    ...
    shiftedMarkup: {
        background: {
            ...
            ";": (markupStack) => {
                ...
                return [
                    null,
                    { specialReset: true, defaultColor: true, bgDefaultColor: true },
                ];
            },
            k: { bgColor: 0 },
            r: { bgColor: 1 },
...
export const characterName = (char) => {
    if (asciiSymbolName[char]) {
        return asciiSymbolName[char];
    }
    return char.toUpperCase();
};
export const keyToUserInterfaceName = (key) => {
    if (asciiSymbolName[key]) {
        return asciiSymbolName[key];
    }
    return key.replace(/([A-Za-z0-9])([A-Za-z0-9]*)|([_-]+)/g, (match, firstLetter, endOfWord, separator) => {
        if (separator) {
            return "-";
        }
...
export const userInterfaceNameToKey = (key) => {
    return key.replace(/([A-Za-z0-9]+)|([_-]+)/g, (match, word, separator) => {
        if (separator) {
            return "_";
        }
...
export { CATCH_ALL_KEYWORDS as markupCatchAllKeywords };
