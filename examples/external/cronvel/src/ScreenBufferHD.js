import gnome from "./colorScheme/gnome.json" with { type: "json" };
...
ScreenBufferHD.createFromString = function (options, data) {
    ...
    if (typeof data !== "string") {
        if (!data.toString) {
            throw new Error("[terminal] ScreenBufferHD.createFromDataString(): argument #1 should be a string or provide a .toString() method.");
        }
        data = data.toString();
    }
    ...
    attr =
        options.attr !== undefined
            ? options.attr
            : ScreenBufferHD.prototype.DEFAULT_ATTR;
    if (attr && typeof attr === "object" && !attr.BYTES_PER_ELEMENT) {
        attr = ScreenBufferHD.object2attr(attr);
    }
    attrTrans = attr;
    if (options.transparencyChar) {
        if (!options.transparencyType) {
            attrTrans |= ScreenBufferHD.prototype.TRANSPARENCY;
        }
        else {
            attrTrans |=
                options.transparencyType & ScreenBufferHD.prototype.TRANSPARENCY;
        }
    }
...
var colorScheme = gnome.map((o) => ({
    color: { r: o.r, g: o.g, b: o.b },
}));
var bgColorScheme = colorScheme.map((o) => ({
    bgColor: { r: o.r, g: o.g, b: o.b },
}));
...
ScreenBufferHD.prototype.blitterCellBlendingIterator = function (p) {
    ...
    if (typeof p.context.blending === "object") {
        if (p.context.blending.fn) {
            blendFn = p.context.blending.fn;
        }
        if (p.context.blending.opacity !== undefined) {
            opacity = p.context.blending.opacity;
        }
        if (p.context.blending.blendSrcFgWithDstBg) {
            blendSrcFgWithDstBg = true;
        }
    }
    ...
    if (attr[BPOS_A]) {
        ...
        if (blendSrcFgWithDstBg) {
            p.context.dstBuffer[p.dstStart + BPOS_R] = alphaBlend(p.context.srcBuffer[p.srcStart + BPOS_R], p.context.dstBuffer[p.dstStart + BPOS_BG_R], alpha, blendFn);
            p.context.dstBuffer[p.dstStart + BPOS_G] = alphaBlend(p.context.srcBuffer[p.srcStart + BPOS_G], p.context.dstBuffer[p.dstStart + BPOS_BG_G], alpha, blendFn);
            p.context.dstBuffer[p.dstStart + BPOS_B] = alphaBlend(p.context.srcBuffer[p.srcStart + BPOS_B], p.context.dstBuffer[p.dstStart + BPOS_BG_B], alpha, blendFn);
            // Blending alpha is special
            p.context.dstBuffer[p.dstStart + BPOS_A] = alphaBlend(p.context.srcBuffer[p.srcStart + BPOS_A], p.context.dstBuffer[p.dstStart + BPOS_BG_A], opacity, ScreenBufferHD.blendFn.screen);
        }
        else {
            p.context.dstBuffer[p.dstStart + BPOS_R] = alphaBlend(p.context.srcBuffer[p.srcStart + BPOS_R], p.context.dstBuffer[p.dstStart + BPOS_R], alpha, blendFn);
            p.context.dstBuffer[p.dstStart + BPOS_G] = alphaBlend(p.context.srcBuffer[p.srcStart + BPOS_G], p.context.dstBuffer[p.dstStart + BPOS_G], alpha, blendFn);
            p.context.dstBuffer[p.dstStart + BPOS_B] = alphaBlend(p.context.srcBuffer[p.srcStart + BPOS_B], p.context.dstBuffer[p.dstStart + BPOS_B], alpha, blendFn);
            // Blending alpha is special
            p.context.dstBuffer[p.dstStart + BPOS_A] = alphaBlend(p.context.srcBuffer[p.srcStart + BPOS_A], p.context.dstBuffer[p.dstStart + BPOS_A], opacity, ScreenBufferHD.blendFn.screen);
        }
    }
    if (attr[BPOS_BG_A]) {
        alpha = (opacity * attr[BPOS_BG_A]) / 255;
        p.context.dstBuffer[p.dstStart + BPOS_BG_R] = alphaBlend(p.context.srcBuffer[p.srcStart + BPOS_BG_R], p.context.dstBuffer[p.dstStart + BPOS_BG_R], alpha, blendFn);
        p.context.dstBuffer[p.dstStart + BPOS_BG_G] = alphaBlend(p.context.srcBuffer[p.srcStart + BPOS_BG_G], p.context.dstBuffer[p.dstStart + BPOS_BG_G], alpha, blendFn);
        p.context.dstBuffer[p.dstStart + BPOS_BG_B] = alphaBlend(p.context.srcBuffer[p.srcStart + BPOS_BG_B], p.context.dstBuffer[p.dstStart + BPOS_BG_B], alpha, blendFn);
        // Blending alpha is special
        p.context.dstBuffer[p.dstStart + BPOS_BG_A] = alphaBlend(p.context.srcBuffer[p.srcStart + BPOS_BG_A], p.context.dstBuffer[p.dstStart + BPOS_BG_A], opacity, ScreenBufferHD.blendFn.screen);
    }
    if (!(attr[BPOS_MISC] & STYLE_TRANSPARENCY)) {
...
function attrEquals(attr1, attr2) {
    if (attr1.readUInt32BE(BPOS_FG) !== attr2.readUInt32BE(BPOS_FG)) {
        return false;
    }
    if (attr1.readUInt32BE(BPOS_BG) !== attr2.readUInt32BE(BPOS_BG)) {
        return false;
    }
    if (attr1[BPOS_STYLE] !== attr2[BPOS_STYLE]) {
        return false;
    }
    if ((attr1[BPOS_MISC] & MISC_ATTR_MASK) !==
        (attr2[BPOS_MISC] & MISC_ATTR_MASK)) {
        return false;
    }
    return true;
}
ScreenBufferHD.prototype.terminalBlitterLineIterator = function (p) {
    ...
    for (offset = p.srcStart; offset < p.srcEnd; offset += this.ITEM_SIZE) {
        ...
        if (!p.context.lastAttr || !attrEquals(attr, p.context.lastAttr)) {
            p.context.sequence +=
                !p.context.lastAttr || !p.context.deltaEscapeSequence
...
ScreenBufferHD.prototype.terminalBlitterCellIterator = function (p) {
    ...
    if (p.context.srcLastBuffer) {
        if (attrEquals(attr, this.readAttr(p.context.srcLastBuffer, p.srcStart)) &&
...
ScreenBufferHD.fromNdarrayImage = function (pixels /*, options */) {
    var x, xMax = pixels.shape[0], y, yMax = Math.ceil(pixels.shape[1] / 2), hasAlpha = pixels.shape[2] === 4;
    ...
    if (style !== lastStyle) {
        ...
        if ((style & BOLD_DIM) !== (lastStyle & BOLD_DIM)) {
            if ((lastStyle & BOLD && !(style & BOLD)) ||
                ...
                if (style & BOLD && !(lastStyle & BOLD)) {
                    esc += term.optimized.bold;
                }
                if (style & DIM && !(lastStyle & DIM)) {
                    esc += term.optimized.dim;
                }
            }
        }
        ...
        if ((style & INVERSE) !== (lastStyle & INVERSE)) {
            esc +=
                style & INVERSE ? term.optimized.inverse : term.optimized.noInverse;
        }
...
ScreenBufferHD.object2attr = function (object) {
    ...
    if (typeof object.bgColor === "string") {
        ...
        attr[BPOS_BG_A] =
            object.bgColor.a !== undefined ? +object.bgColor.a || 0 : 255;
    }
    else {
        attr[BPOS_BG_R] = 0;
        attr[BPOS_BG_G] = 0;
    ...
    attr[BPOS_STYLE] = 0;
    if (object.bold) {
        attr[BPOS_STYLE] |= BOLD;
    }
    if (object.dim) {
        attr[BPOS_STYLE] |= DIM;
    }
    if (object.italic) {
        attr[BPOS_STYLE] |= ITALIC;
    }
    if (object.underline) {
        attr[BPOS_STYLE] |= UNDERLINE;
    }
    if (object.blink) {
        attr[BPOS_STYLE] |= BLINK;
    }
    if (object.inverse) {
        attr[BPOS_STYLE] |= INVERSE;
    }
    if (object.hidden) {
        attr[BPOS_STYLE] |= HIDDEN;
    }
    if (object.strike) {
        attr[BPOS_STYLE] |= STRIKE;
    }
    return attr;
};
...
ScreenBufferHD.attrAndObject = function (attr, object) {
    ...
    if (object.color && typeof object.color === "object") {
        if (object.color.r !== undefined) {
            attr[BPOS_R] = +object.color.r || 0;
        }
        if (object.color.g !== undefined) {
            attr[BPOS_G] = +object.color.g || 0;
        }
        if (object.color.b !== undefined) {
            attr[BPOS_B] = +object.color.b || 0;
        }
        if (object.color.a !== undefined) {
            attr[BPOS_A] = +object.color.a || 0;
        }
    }
    if (object.bgColor && typeof object.bgColor === "object") {
        if (object.bgColor.r !== undefined) {
            attr[BPOS_BG_R] = +object.bgColor.r || 0;
        }
        if (object.bgColor.g !== undefined) {
            attr[BPOS_BG_G] = +object.bgColor.g || 0;
        }
        if (object.bgColor.b !== undefined) {
            attr[BPOS_BG_B] = +object.bgColor.b || 0;
        }
        if (object.bgColor.a !== undefined) {
            attr[BPOS_BG_A] = +object.bgColor.a || 0;
        }
    }
...
ScreenBufferHD.attrInverse = ScreenBufferHD.prototype.attrInverse = (attr) => {
    attr[BPOS_STYLE] ^= INVERSE;
    return attr;
};
/* Constants */
...
ScreenBufferHD.prototype.ITEM_SIZE =
    ScreenBufferHD.prototype.ATTR_SIZE + ScreenBufferHD.prototype.CHAR_SIZE;
...
ScreenBufferHD.loadSyncV2 = function (filepath) {
    ...
    if (header.bitsPerColor &&
        header.bitsPerColor !== ScreenBufferHD.prototype.bitsPerColor) {
        throw new Error("Bad Bits Per Color: " +
            header.bitsPerColor +
            " (should be " +
            ScreenBufferHD.prototype.bitsPerColor +
            ")");
    }
    // Bad size?
    if (content.length !==
        i + 1 + header.width * header.height * ScreenBufferHD.prototype.ITEM_SIZE) {
