...
TextBuffer.prototype.removeTrailingSpaces = function (y = this.cy, x = null, dry = false) {
    ...
    var deletedStr = "", hasNL = line[x].char === "\n";
    if (hasNL) {
        x--;
    }
    for (; x >= 0; x--) {
...
TextBuffer.prototype.reTabLine = function (startAt = 0, y = this.cy) {
    var length, cell, index, fillSize, input, output, linePosition = startAt;
...
TextBuffer.prototype.wrapLine = function (startY = this.cy, width = this.lineWrapWidth, wordWrap = this.wordWrap) {
    var x, y, rightShift, endY, line, lineWidth, previousLine, lastChar, found, cursorInlineOffset, checkCursor = this.cy === startY;
...
TextBuffer.prototype.moveToLine = TextBuffer.prototype.moveToRow = function (y) {
...
TextBuffer.prototype.moveRight = function () {
    this.cx++;
    var currentLine = this.buffer[this.cy];
    while (currentLine?.[this.cx]?.filler &&
        currentLine?.[this.cx]?.width === 0) {
        this.cx++;
    }
    if (this.forceInBound) {
...
TextBuffer.prototype.moveForward = function (testFn, justSkipFiller) {
    var oldCx = this.cx, currentLine = this.buffer[this.cy];
...
TextBuffer.prototype.moveBackward = function (testFn, justSkipFiller) {
    var lineLength, currentLine = this.buffer[this.cy];
    //if ( justSkipFiller && ( ! currentLine || ! currentLine[ this.cx ] || ! currentLine[ this.cx ].filler || currentLine[ this.cx ].char !== '\n' ) ) { return ; }
    if (justSkipFiller &&
        (!currentLine || !currentLine[this.cx] || !currentLine[this.cx].filler)) {
        return;
...
TextBuffer.prototype.insert = function (text, hasMarkup, attr) {
    var lines, index, length, count = 0;
    if (!text) {
        return count;
    }
    if (typeof hasMarkup !== "boolean" && typeof hasMarkup !== "string") {
...
TextBuffer.prototype.delete = function (count, getDeleted = false) {
    var currentLine, inlineCount, fillerCount, hasNL, removedCells, deleted = getDeleted ? { string: "", count: 0 } : undefined;
    if (count === undefined) {
        count = 1;
    }
    if (this.forceInBound) {
...
TextBuffer.prototype.backDelete = function (count, getDeleted = false) {
    ...
    var currentLine, inlineCount, fillerCount, tabIndex, removedCells, deleted = getDeleted ? { string: "", count: 0 } : undefined;
    if (count === undefined) {
        count = 1;
    }
    if (this.forceInBound) {
...
TextBuffer.prototype.countInlineForwardFiller = function (count) {
    var x, cell, xMax = this.buffer[this.cy].length, filler = 0;
...
TextBuffer.prototype.joinLine = function (internalCall, y) {
    var tabIndex, currentLine, x, updateCursor = false, hasDeleted = false;
    if (y === undefined) {
        y = this.cy;
        updateCursor = true;
    }
...
TextBuffer.prototype.findNext = function (searchString, startPosition, reverse) {
    var index, startAt, endAt, text = this.getText(), 
...
TextBuffer.prototype.regexpFindNext = function (regexp, startPosition = 0) {
    var index, startAt, endAt, size, match, text = this.getText();
...
TextBuffer.prototype.drawCursor = function (options) {
    ...
    var cx, dst = options.dst || this.dst;
    if (dst instanceof this.ScreenBuffer) {
        cx = this.cy ? this.cx : this.cx + this.firstLineRightShift;
        if (!this.ch &&
            (this.dstClipRect || new termkit.Rect(this.dst)).isInside(cx + this.x, this.cy + this.y)) {
            dst.cx = cx + this.x;
...
TextBuffer.prototype.blitter = function (p) {
    ...
    tr = {
        ...
        context: {
            ...
            writeAttr: this.ScreenBuffer === termkit.ScreenBuffer
                ? (dst, attr, offset) => {
