...
function Button(options) {
  ...
  this.disabledAttr = options.disabledAttr || {
    bgColor: "black",
    color: "brightBlack",
  };
  this.submittedAttr = options.submittedAttr || { bgColor: "brightBlue" };
  this.turnedOnBlurAttr = options.turnedOnBlurAttr || { bgColor: "cyan" };
  this.turnedOnFocusAttr = options.turnedOnFocusAttr || {
    bgColor: "brightCyan",
    color: "gray",
    bold: true,
  };
  this.turnedOffBlurAttr = options.turnedOffBlurAttr || {
    bgColor: "gray",
    dim: true,
  };
  this.turnedOffFocusAttr = options.turnedOffFocusAttr || {
    bgColor: "white",
    color: "black",
    bold: true,
  };
  ...
  if (options.actionKeyBindings) {
    this.actionKeyBindings = options.actionKeyBindings;
  }
  ...
  if (this.elementType === "Button" && !options.noDraw) {
    this.draw();
  }
}
...
Button.prototype._toContentArray = function (content) {
  return !this.forceContentArray || Array.isArray(content)
    ? content
    : [content || ""];
};

Button.prototype.setContent = function (
  content,
  hasMarkup,
  dontDraw = false,
  dontResize = false,
  ...
  if (!content || typeof content !== "object" || Array.isArray(content)) {
    ...
    if (content.internal) {
      ...
      this.blurContent = this._toContentArray(
        content.blurContent ?? this.content,
      );
      this.focusContent = this._toContentArray(
        content.focusContent ?? this.content,
      );
      this.disabledContent = this._toContentArray(
        content.disabledContent ?? this.content,
      );
      this.submittedContent = this._toContentArray(
        content.submittedContent ?? this.content,
      );
      this.turnedOnBlurContent = this._toContentArray(
        content.turnedOnBlurContent ?? content.turnedOnContent ?? this.content,
      );
      this.turnedOffBlurContent = this._toContentArray(
        content.turnedOffBlurContent ??
          content.turnedOffContent ??
          this.content,
      );
      this.turnedOnFocusContent = this._toContentArray(
        content.turnedOnFocusContent ?? content.turnedOnContent ?? this.content,
      );
      this.turnedOffFocusContent = this._toContentArray(
        content.turnedOffFocusContent ??
          content.turnedOffContent ??
          this.content,
      );
    } else {
      ...
      this.disabledContent = this._toContentArray(
        content.disabled ?? this.content,
      );
      this.submittedContent = this._toContentArray(
        content.submitted ?? this.content,
      );
      this.turnedOnBlurContent = this._toContentArray(
        content.turnedOnBlur ?? content.turnedOn ?? this.content,
      );
      this.turnedOffBlurContent = this._toContentArray(
        content.turnedOffBlur ?? content.turnedOff ?? this.content,
      );
      this.turnedOnFocusContent = this._toContentArray(
        content.turnedOnFocus ?? content.turnedOn ?? this.content,
      );
      this.turnedOffFocusContent = this._toContentArray(
        content.turnedOffFocus ?? content.turnedOff ?? this.content,
      );
    }
...
Button.prototype.computeRequiredWidth = function () {
  ...
  this.innerWidth =
    this.leftPaddingWidth + this.rightPaddingWidth + this.contentWidth || 1;
