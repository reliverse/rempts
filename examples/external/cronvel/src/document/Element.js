...
function Element(options = {}) {
    ...
    this.parent =
        options.parent && options.parent.elementType ? options.parent : null;
    ...
    this.strictInline = !!(this.inlineTerm &&
        this.strictInlineSupport &&
        (options.strictInline || options.strictInline === undefined));
    this.restoreCursorAfterDraw = !!(this.inlineTerm &&
        this.inlineCursorRestoreAfterDraw &&
        !this.strictInline);
    (this.outputDst =
        options.outputDst || (options.parent && options.parent.inputDst)),
        (this.inputDst = null);
    ...
    this.outputWidth =
        this.autoWidth && this.outputDst
            ? Math.round(this.outputDst.width * this.autoWidth)
            : options.outputWidth
                ? options.outputWidth
                : options.width
                    ? options.width
                    : this.strictInline
                        ? this.inlineTerm.width
                        : 1;
    this.outputHeight =
        this.autoHeight && this.outputDst
            ? Math.round(this.outputDst.height * this.autoHeight)
            : options.outputHeight
                ? options.outputHeight
                : options.height
                    ? options.height
                    : this.strictInline
                        ? this.inlineTerm.height
                        : 1;
    this.contentAdaptativeWidth =
        this.contentAdaptativeWidth ?? !!options.contentAdaptativeWidth;
    this.contentAdaptativeHeight =
        this.contentAdaptativeHeight ?? !!options.contentAdaptativeHeight;
    // Used by .updateDraw()
    ...
    if (this.parent) {
        this.parent.attach(this, options.id);
    }
    if (options.shortcuts && this.document) {
        if (Array.isArray(options.shortcuts)) {
            this.document.createShortcuts(this, ...options.shortcuts);
        }
        else {
            this.document.createShortcuts(this, options.shortcuts);
        }
...
Element.prototype.destroy = function (isSubDestroy = false, noDraw = false) {
    ...
    if (!isSubDestroy) {
        this.detach(noDraw);
        if (this.inlineTerm && document !== this) {
            document.destroy();
        }
    }
    else {
...
// User API
Element.prototype.destroyNoRedraw = function () {
    return this.destroy(undefined, true);
};
...
// Debug function
Element.prototype.debugId = function () {
    return this.elementType + "#" + this.uid;
};
...
Element.prototype.attach = function (child, id) {
    ...
    if (this.children.indexOf(child) === -1) {
        ...
        if (!child.inputDst) {
            child.inputDst = child.outputDst;
        }
...
Element.prototype.attachTo = function (parent, id) {
    if (parent.elementType) {
        parent.attach(this, id);
    }
    return this;
};
Element.prototype.recursiveFixAttachment = function (document, id = this.id) {
    ...
    if (document) {
        document.assignId(this, id);
    }
    else if (this.document) {
        this.document.unassignId(this, this.id);
    } // force actual id here
    else {
        this.id = null;
    }
...
Element.prototype.detach = function (noDraw = false) {
    ...
    if (index >= 0) {
        parent.zChildren.splice(index, 1);
    }
    delete this.document.elements[this.id];
    this.parent = null;
...
Element.prototype.zInsert = function (child) {
    var current, i = this.zChildren.length;
    while (i--) {
...
Element.prototype.topZ = function () {
    if (this.parent.interceptTempZIndex) {
        return this.parent.topZ();
    }
    if (!this.parent.zChildren.length) {
        return;
    }
    this.zIndex =
        this.parent.zChildren[this.parent.zChildren.length - 1].zIndex + 1;
    this.parent.zSort();
};
// Change zIndex to make it on bottom of all siblings
Element.prototype.bottomZ = function () {
    if (this.parent.interceptTempZIndex) {
        return this.parent.bottomZ();
    }
    if (!this.parent.zChildren.length) {
        return;
    }
...
Element.computeContentWidth = (content, hasMarkup) => {
    if (Array.isArray(content)) {
        return hasMarkup === "ansi" || hasMarkup === "legacyAnsi"
            ? Math.max(...content.map((line) => misc.ansiWidth(line)))
            : hasMarkup
                ? Math.max(...content.map((line) => misc.markupWidth(line)))
                : Math.max(...content.map((line) => string.unicode.width(line)));
    }
    return hasMarkup === "ansi" || hasMarkup === "legacyAnsi"
        ? misc.ansiWidth(content)
        : hasMarkup
            ? misc.markupWidth(content)
...
Element.wordwrapContent = // <-- DEPRECATED
    Element.wordWrapContent = (content, width, hasMarkup) => hasMarkup === "ansi" || hasMarkup === "legacyAnsi"
        ? misc.wordWrapAnsi(content, width)
        : hasMarkup
            ? misc.wordWrapMarkup(content, width)
...
Element.prototype.setContent = function (content, hasMarkup, dontDraw = false, dontResize = false) {
    ...
    var oldOutputWidth = this.outputWidth, oldOutputHeight = this.outputHeight;
    ...
    if (!dontDraw) {
        if (this.outputWidth < oldOutputWidth ||
...
Element.prototype.getParentContainer = function () {
    ...
    for (;;) {
        ...
        if (currentElement.parent.isContainer) {
...
Element.prototype.focusNextChild = function (loop = true, type = "cycle") {
    ...
    for (;;) {
        ...
        if (focusAware || startingIndex === index) {
            break;
        }
    }
...
Element.prototype.childrenAt = function (x, y, filter = null, matches = []) {
    ...
    while (i--) {
        ...
        if (x >= current.outputX &&
            x <= current.outputX + current.outputWidth - 1 &&
            y >= current.outputY &&
            y <= current.outputY + current.outputHeight - 1) {
            // Bounding box match!
...
Element.prototype.draw = function (isInitialInlineDraw = false) {
    ...
    if (!this.document || this.hidden) {
        return this;
    }
    if (!isInitialInlineDraw) {
        if (this.restoreCursorAfterDraw) {
            this.inlineTerm.saveCursor();
        }
        else if (!this.strictInline) {
            this.saveCursor();
        }
    }
    this.descendantDraw();
...
Element.prototype.redraw = // DEPRECATED name, use .outerDraw()
    Element.prototype.outerDraw = function (force = false) {
        if (!this.document || (this.hidden && !force)) {
            return this;
        }
...
Element.prototype.ascendantDraw = function () {
    ...
    while (currentElement.parent &&
        currentElement.outputDst !== currentElement.document.outputDst) {
        currentElement = currentElement.parent;
        if (currentElement.outputDst !== currentElement.inputDst &&
            currentElement.postDrawSelf &&
...
Element.prototype.drawCursor = function () {
    ...
    currentElement = this;
    while (currentElement.outputDst !== currentElement.document.outputDst &&
...
// TODOC
Element.prototype.bindKey = function (key, action) {
    this.keyBindings[key] = action;
};
// TODOC
Element.prototype.getKeyBinding = function (key) {
...
Element.createInline = async function (term, Type, options) {
    ...
    element.draw(true);
    term.styleReset();
    if (element.staticInline) {
        element.destroy(undefined, true);
    }
    return element;
};
...
Element.prototype.onKey = function (key, trash, data) {
    ...
    if (action) {
        if (action === "meta") {
            ...
            return ((this.userActions[action].call(this, key, trash, data) ?? true) ||
                undefined);
        }
    }
        ...
        if (this.userActions.character) {
            // Do not bubble up except if explicitly false
            return ((this.userActions.character.call(this, key, trash, data) ?? true) ||
                undefined);
        }
    }
    else if (this.userActions.specialKey) {
        // Do not bubble up except if explicitly false
        return ((this.userActions.specialKey.call(this, key, trash, data) ?? true) ||
            undefined);
    }
