...
function Layout(options) {
  ...
  if (options.boxChars) {
    if (typeof options.boxChars === "object") {
      ...
      typeof options.boxChars === "string" &&
      boxesChars[options.boxChars]
    ) {
  ...
  if (this.elementType === "Layout" && !options.noDraw) {
    this.draw();
  }
}
...
Layout.prototype.computeBoundingBoxes_ = function (
  layoutDef,
  computed,
  parent,
  inProgress,
  ...
  if (layoutDef.columns && layoutDef.columns.length) {
    ...
    for (i = 0; i < layoutDef.columns.length; i++) {
      ...
      if (computed.columns[i].dx_ !== undefined) {
        nextInProgress.remainingDx -= computed.columns[i].dx_;
      } else {
        nextInProgress.autoDxCount++;
      }
    }

    for (i = 0; i < layoutDef.columns.length; i++) {
      this.computeBoundingBoxes_(
        layoutDef.columns[i],
        computed.columns[i],
        computed,
        nextInProgress,
    ...
    for (i = 0; i < layoutDef.rows.length; i++) {
      ...
      if (computed.rows[i].dy_ !== undefined) {
        nextInProgress.remainingDy -= computed.rows[i].dy_;
      } else {
        nextInProgress.autoDyCount++;
      }
    }
  ...
  if (!hasChild) {
    if (this.boxesContainer[layoutDef.id]) {
      if (
        this.boxesContainer[layoutDef.id].width !== computed.width - 2 ||
        this.boxesContainer[layoutDef.id].height !== computed.height - 2
      ) {
...
Layout.prototype.computeDxDy = function (
  layoutDef,
  computed,
  parent,
  inProgress,
  ...
  if (firstPass || computed.dx_ === undefined) {
    if (layoutDef.width !== undefined) {
      ...
      computed.dx_ = Math.max(
        0,
        Math.min(parent.dx_, (parent.dx_ * layoutDef.widthPercent) / 100),
      );
    } else if (!firstPass) {
      //console.error( ">>>>>>>>>> #" + layoutDef.id + ' remaining dx: ' , inProgress.remainingDx , '/' , inProgress.autoDxCount , ' --- ' , inProgress ) ;
      computed.dx_ = Math.max(
        0,
        inProgress.remainingDx / (inProgress.autoDxCount || 1),
      );
  ...
  if (firstPass || computed.dy_ === undefined) {
    if (layoutDef.height !== undefined) {
      ...
      computed.dy_ = Math.max(
        0,
        Math.min(parent.dy_, (parent.dy_ * layoutDef.heightPercent) / 100),
      );
    } else if (!firstPass) {
      computed.dy_ = Math.max(
        0,
        inProgress.remainingDy / (inProgress.autoDyCount || 1),
      );
    }
  }
};
...
Layout.prototype.preDrawSelf = function () {
  ...
  this.outputDst.put(
    ...
    this.boxChars.topLeft +
      this.boxChars.horizontal.repeat(this.computed.dx - 1) +
...
Layout.prototype.drawRow = function (computed, tees, last) {
  if (!last) {
    ...
    // Draw the bottom border
    this.outputDst.put(
      { x: computed.xmin + 1, y: computed.ymax },
