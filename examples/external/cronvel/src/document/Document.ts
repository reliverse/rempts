...
function Document(options) {
  ...
  this.getSystemClipboard = Promise.debounceDelay(500, async (source) => {
    if (!this.outputDst.getClipboard) {
      return "";
    }
  ...
  if (this.elementType === "Document" && !options.noDraw) {
    this.draw();
  }
}
...
Document.prototype.keyBindings = Object.assign(
  {},
...
Document.prototype.giveFocusTo = function (element, type = "direct") {
  if (!(element instanceof Element)) {
    throw new TypeError("" + element + " is not an instance of Element.");
  }
  if (this.isAncestorOf(element)) {
    return this.giveFocusTo_(element, type);
  }
};
...
Document.prototype.giveFocusTo_ = function (element, type) {
  ...
  if (focusAware) {
    ...
    while (ancestor) {
      if (ancestor.listenerCount("clickOut")) {
        this.clickOutCandidates.add(ancestor);
      }
...
Document.prototype.focusNext = function () {
  ...
  if (!this.focusElement || !this.isAncestorOf(this.focusElement)) {
    currentElement = this;
  } else {
    currentElement = this.focusElement;
  }

  if (currentElement === this && !this.children.length) {
    return;
  }
  ...
  for (;;) {
    if (currentElement.children.length && !currentElement.noChildFocus) {
      ...
      for (;;) {
        ...
        if (index + 1 < currentElement.parent.children.length) {
          ...
          if (startingElement === currentElement.parent) {
            return;
          }
    ...
    if (
      startingElement === currentElement ||
      (!currentElement.hidden && focusAware)
    ) {
      break;
    }
  }
  ...
  for (;;) {
    if (currentElement.parent) {
      ...
      if (index - 1 >= 0) {
        ...
        if (!currentElement.hidden) {
          focusAware = this.giveFocusTo_(currentElement, "backCycle");
        }
      } else if (currentElement.parent.parent) {
        currentElement = currentElement.parent;
        if (!currentElement.hidden) {
        ...
        currentElement =
          currentElement.parent.children[
            currentElement.parent.children.length - 1
          ];
...
Document.prototype.bubblingEvent = function (element, key, altKeys, data) {
  if (element !== this) {
    element.emit("key", key, altKeys, data, (interruption, event) => {
      ...
      if (!interruption) {
        if (element.parent) {
          this.bubblingEvent(element.parent, key, altKeys, data);
        } else {
          this.defaultKeyHandling(key, altKeys, data);
        }
      }
...
Document.prototype.defaultKeyHandling = function (key, altKeys, data) {
  switch (this.keyBindings[key]) {
    ...
    default:
      if (
        this.elementByShortcut[key] &&
        this.elementByShortcut[key].document === this
      ) {
...
// TODOC
Document.prototype.setMetaKeyPrefix = function (prefix, remove) {
  this.eventSource.setMetaKeyPrefix(prefix, remove);
};
Document.prototype.getDocumentClipboard = function (key = "content") {
  return this.documentClipboards[key];
};
Document.prototype.setDocumentClipboard = function (value, key = "content") {
  this.documentClipboards[key] = "" + value;
};
Document.prototype.clearDocumentClipboard = function (value, key = "content") {
  delete this.documentClipboards[key];
};
...
Document.prototype.removeElementShortcuts = function (element) {
  for (let key in this.elementByShortcut) {
    if (this.elementByShortcut[key] === element) {
      this.elementByShortcut[key] = null;
    }
  }
};
...
const COMMON_MOUSE_AWARE_FILTER = (element) =>
  element.listenerCount("click") ||
  element.listenerCount("clickOut") ||
  element.listenerCount("rightClick") ||
  element.listenerCount("middleClick") || //element.listenerCount( 'wheel' ) ||
  element.listenerCount("dragStart") ||
  element.listenerCount("drag") ||
  element.listenerCount("dragEnd") ||
  element.listenerCount("hover") ||
  element.listenerCount("leave") ||
  element.listenerCount("enter");
...
Document.prototype.mouseClick = function (data, clickType = "click") {
  ...
  if (this.clickOutCandidates.size) {
    for (let candidate of this.clickOutCandidates) {
      ...
      if (
        candidate.document === this &&
        candidate !== matches[0].element &&
        !candidate.isAncestorOf(matches[0].element)
      ) {
        candidate.emit("clickOut");
      }
    }
  ...
  matches[0].element.emit(
    clickType,
    { x: matches[0].x, y: matches[0].y },
    matches[0].element,
  );
};
...
Document.prototype.mouseMotion = function (data, exclude = null) {
  ...
  if (!starting && !this.motionData.dx && !this.motionData.dy) {
    return;
  }
...
Document.prototype.mouseDrag = function (data) {
  ...
  if (!starting && !this.draggingData.dx && !this.draggingData.dy) {
    return;
  }
...
Document.prototype.mouseDragStart = function (data) {
  ...
  matches = this.childrenAt(
    data.xFrom - this.outputX,
    data.yFrom - this.outputY,
    COMMON_MOUSE_AWARE_FILTER,
  );
...
Document.prototype.mouseWheel = function (data) {
  ...
  var matches = this.childrenAt(
    data.x - this.outputX,
    data.y - this.outputY,
    (element) => element.listenerCount("wheel"),
  );
  if (!matches.length) {
    return;
  }
  matches[0].element.emit(
    "wheel",
    { x: matches[0].x, y: matches[0].y, yDirection: data.yDirection },
    matches[0].element,
  );
};
