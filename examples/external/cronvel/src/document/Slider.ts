...
function Slider(options) {
  ...
  this.rateToValue =
    typeof options.rateToValue === "function" ? options.rateToValue : IDENTITY;
  this.valueToRate =
    typeof options.valueToRate === "function" ? options.valueToRate : IDENTITY;
  ...
  this.handleAttr = options.handleAttr || {
    bgColor: "brightWhite",
    color: "black",
  };
  ...
  if (this.elementType === "Slider" && !options.noDraw) {
    this.draw();
  }
}
...
Slider.prototype.initChildren = function () {
  ...
  this.forwardButton = new Button({
    ...
    outputX: this.isVertical
      ? this.outputX
      : this.outputX + this.outputWidth - 1,
    outputY: this.isVertical
      ? this.outputY + this.outputHeight - 1
      : this.outputY,
    blurAttr: this.buttonBlurAttr,
    focusAttr: this.buttonFocusAttr,
    //disabledAttr: this.buttonDisabledAttr ,
...
Slider.prototype.setSizeAndPosition = function (options) {
  ...
  this.forwardButton.outputX = this.isVertical
    ? this.outputX
    : this.outputX + this.outputWidth - 1;
  this.forwardButton.outputY = this.isVertical
    ? this.outputY + this.outputHeight - 1
    : this.outputY;
};
...
Slider.prototype.preDrawSelf = function () {
  return this.isVertical
    ? this.preDrawSelfVertical()
    : this.preDrawSelfHorizontal();
};
...
Slider.prototype.preDrawSelfVertical = function () {
  ...
  for (; y <= yMax; y++, offset++) {
    if (offset === this.handleOffset) {
      this.outputDst.put(
        { x: this.outputX, y, attr: this.handleAttr },
        this.handleSymbol,
      );
    } else {
      this.outputDst.put(
        { x: this.outputX, y, attr: this.barAttr },
        this.barSymbol,
      );
    }
  }
...
Slider.prototype.preDrawSelfHorizontal = function () {
  ...
  for (; x <= xMax; x++, offset++) {
    if (offset === this.handleOffset) {
      this.outputDst.put(
        { x, y: this.outputY, attr: this.handleAttr },
        this.handleSymbol,
      );
    } else {
      this.outputDst.put(
        { x, y: this.outputY, attr: this.barAttr },
        this.barSymbol,
      );
    }
  }
...
Slider.prototype.setHandleOffset = function (
  offset,
  internalAndNoDraw = false,
) {
  var delta = (this.isVertical ? this.outputHeight : this.outputWidth) - 3; // minus the two buttons
...
Slider.prototype.getHandleOffset = function () {
  return this.handleOffset;
};
Slider.prototype.getSlideRate = function () {
