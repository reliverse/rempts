...
Element.wordwrapContent = // <-- DEPRECATED
  Element.wordWrapContent = (content, width, hasMarkup) =>
    hasMarkup === "ansi" || hasMarkup === "legacyAnsi"
...
Element.prototype.setContent = function (
  ...
  this.contentWidth = Element.computeContentWidth(
    content,
...
Element.prototype.ascendantDraw = function () {
  ...
  while (
    ...
    if (
      currentElement.outputDst !== currentElement.inputDst &&
...
Element.prototype.onKey = function (key, trash, data) {
  ...
  if (action) {
    if (action === "meta") {
      ...
      return (
        (this.userActions[action].call(this, key, trash, data) ?? true) ||
        undefined
      );
    }
    ...
    if (this.userActions.character) {
      // Do not bubble up except if explicitly false
      return (
        (this.userActions.character.call(this, key, trash, data) ?? true) ||
        undefined
      );
    }
    ...
    // Do not bubble up except if explicitly false
    return (
      (this.userActions.specialKey.call(this, key, trash, data) ?? true) ||
      undefined
    );
  }
