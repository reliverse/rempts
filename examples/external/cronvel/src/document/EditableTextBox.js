...
function EditableTextBox(options) {
    ...
    this.editionUpdateDebounced = this.debounceTimeout
        ? Promise.debounceUpdate({ delay: this.debounceTimeout }, this.editionUpdate)
        : this.editionUpdate;
    ...
    this.extraScrolling =
        options.extraScrolling !== undefined ? !!options.extraScrolling : true;
    this.updateStatus();
    ...
    if (this.elementType === "EditableTextBox" && !options.noDraw) {
        this.draw();
    }
}
...
EditableTextBox.prototype.onDragEnd = function (data) {
    if (this.hasFocus) {
        if (data.yFrom < data.y ||
            (data.yFrom === data.y && data.xFrom <= data.x)) {
...
userActions.character = function (key) {
    var x = this.textBuffer.cx, y = this.textBuffer.cy;
...
userActions.newLine = function () {
    var insertedString = "\n", count = 1, x = this.textBuffer.cx, y = this.textBuffer.cy;
...
userActions.delete = function () {
    var x = this.textBuffer.cx, y = this.textBuffer.cy, selectionRegion = this.textBuffer.selectionRegion;
    if (selectionRegion &&
        selectionRegion.ymin === y &&
        selectionRegion.xmin === x) {
...
userActions.expandSelectionBackward = function () {
    var selection = this.textBuffer.selectionRegion, cx = this.textBuffer.cx, cy = this.textBuffer.cy, oneStepBackward = this.textBuffer.oneStepBackward();
    if (selection && selection.xmin === cx && selection.ymin === cy) {
        ...
        selection.xmax === oneStepBackward.x &&
        selection.ymax === oneStepBackward.y) {
        // Can contract
        this.textBuffer.moveBackward();
...
userActions.expandSelectionUp = function () {
    ...
    if (selection && selection.xmin === cx && selection.ymin === cy) {
        ...
        (selection.ymin < oneStepBackward.y - 1 ||
            (selection.ymin === oneStepBackward.y - 1 &&
                selection.xmin <= oneStepBackward.x))) {
        // Can contract
        this.textBuffer.moveUp();
        this.textBuffer.endOfSelection();
    }
...
userActions.expandSelectionDown = function () {
    ...
    if (selection &&
        ...
        selection.ymin === cy &&
        (selection.ymax > cy + 1 ||
            (selection.ymax === cy + 1 && selection.xmax >= cx - 1))) {
        // Can contract
        this.textBuffer.moveDown();
        this.textBuffer.startOfSelection();
    }
...
userActions.clearSystemClipboard = function () {
    if (this.document) {
        this.document
            .clearSystemClipboard(this.textBuffer.getSelectionText())
            .catch(() => undefined);
    }
};
