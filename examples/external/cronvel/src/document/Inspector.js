...
function Inspector(options) {
    ...
    this.onPropertiesColumnMenuPageInit =
        this.onPropertiesColumnMenuPageInit.bind(this);
    this.onPropertiesColumnMenuSubmit =
        this.onPropertiesColumnMenuSubmit.bind(this);
    this.onBreadCrumbRowMenuSubmit = this.onBreadCrumbRowMenuSubmit.bind(this);
    if (!options.outputWidth && !options.width) {
        options.outputWidth = 78;
    }
    ...
    if (options.textInputKeyBindings) {
        this.textInputKeyBindings = options.textInputKeyBindings;
    }
    this.on("key", this.onKey);
    this.on("focus", this.onFocus);
    this.initChildren();
    // Only draw if we are not a superclass of the object
    if (this.elementType === "Inspector" && !options.noDraw) {
        this.draw();
    }
}
...
Inspector.prototype.initChildren = function (noInitLevel = false) {
    var offsetX = 0, offsetY = 0, buttonsTextWidth = 0, buttonSpacing = 0;
    // Submit Button part
    ...
    if (this.elementType === "Inspector" && !noInitLevel) {
        this.initLevel();
    }
};
...
Inspector.prototype.initLevel = function () {
    this.resetChildren();
    var current = this.inspectStack[this.inspectStack.length - 1], subObject = current.object, breadCrumbMenuItems = [], propertiesMenuItems = [], path = this.inspectStack.map((e) => e.key).join(".");
...
Inspector.prototype.createValueFieldTexts = function () {
    var current = this.inspectStack[this.inspectStack.length - 1], subObject = current.object;
    for (let valueFieldText of this.valueFieldTextList) {
        valueFieldText.destroyNoRedraw();
    }
    this.valueFieldTextList.length = 0;
    for (let button of this.propertiesColumnMenu.buttons) {
        if (button.def.internalRole) {
            continue;
        }
        ...
        if (value && typeof value === "object") {
            ...
            if (Array.isArray(value)) {
                ...
                valueFieldTextContent = proto?.constructor?.name
...
Inspector.prototype.resetChildren = function () {
    if (!this.valueFieldTextList.length &&
        !this.propertiesColumnMenu &&
        !this.breadCrumbText) {
        return;
    }
    for (let valueFieldText of this.valueFieldTextList) {
...
Inspector.prototype.onPropertiesColumnMenuSubmit = function (buttonValue, action, menu, button) {
    if (action) {
        ...
        let current = this.inspectStack[this.inspectStack.length - 1], subObject = current.object;
        if (subObject[buttonValue] && typeof subObject[buttonValue] === "object") {
            current.fromKey = buttonValue;
            this.inspectStack.push({
                object: subObject[buttonValue],
                key: buttonValue,
            });
        }
    }
...
Inspector.prototype.onBreadCrumbRowMenuSubmit = function (buttonValue, action, menu, button) {
    if (buttonValue === this.inspectStack.length - 1) {
        return;
    }
