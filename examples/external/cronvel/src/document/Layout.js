...
function Layout(options) {
    ...
    if (options.boxChars) {
        ...
        else if (typeof options.boxChars === "string" &&
...
Layout.prototype.computeBoundingBoxes_ = function (layoutDef, computed, parent, inProgress) {
    ...
    if (!hasChild) {
        if (this.boxesContainer[layoutDef.id]) {
            if (this.boxesContainer[layoutDef.id].width !== computed.width - 2 ||
...
Layout.prototype.preDrawSelf = function () {
    ...
    //this.computeBoundingBoxes() ;
    // Draw the top border
    this.outputDst.put({ x: this.computed.xmin, y: this.computed.ymin }, this.boxChars.topLeft +
        this.boxChars.horizontal.repeat(this.computed.dx - 1) +
        this.boxChars.topRight);
    // Draw the bottom border
    this.outputDst.put({ x: this.computed.xmin, y: this.computed.ymax }, this.boxChars.bottomLeft +
        this.boxChars.horizontal.repeat(this.computed.dx - 1) +
