...
function BaseMenu(options = {}) {
  ...
  this.backgroundAttr = options.backgroundAttr || {
    bgColor: "white",
    color: "black",
  };
  ...
  this.buttonBlurAttr = options.buttonBlurAttr ||
    this.defaultOptions.buttonBlurAttr || {
      bgColor: "black",
      color: "white",
      bold: true,
    };
  this.buttonEvenBlurAttr = options.buttonEvenBlurAttr || null;
  this.buttonFocusAttr = options.buttonFocusAttr ||
    this.defaultOptions.buttonFocusAttr || {
      bgColor: "white",
      color: "black",
      bold: true,
    };
  this.buttonDisabledAttr = options.buttonDisabledAttr ||
    this.defaultOptions.buttonDisabledAttr || {
      bgColor: "black",
      color: "gray",
      bold: true,
    };
  this.buttonSubmittedAttr = options.buttonSubmittedAttr ||
    this.defaultOptions.buttonSubmittedAttr || {
      bgColor: "gray",
      color: "brightWhite",
      bold: true,
    };
  this.buttonTurnedOnBlurAttr = options.buttonTurnedOnBlurAttr ||
    this.defaultOptions.buttonTurnedOnBlurAttr || { bgColor: "cyan" };
  this.buttonTurnedOnFocusAttr = options.buttonTurnedOnFocusAttr ||
    this.defaultOptions.buttonTurnedOnFocusAttr || {
      bgColor: "brightCyan",
      color: "gray",
      bold: true,
    };
  this.buttonTurnedOffBlurAttr = options.buttonTurnedOffBlurAttr ||
    this.defaultOptions.buttonTurnedOffBlurAttr || {
      bgColor: "gray",
      dim: true,
    };
  this.buttonTurnedOffFocusAttr = options.buttonTurnedOffFocusAttr ||
    this.defaultOptions.buttonTurnedOffFocusAttr || {
      bgColor: "white",
      color: "black",
      bold: true,
    };
  ...
  this.blurRightPadding =
    options.blurRightPadding ?? options.rightPadding ?? "";
  this.focusLeftPadding = options.focusLeftPadding ?? options.leftPadding ?? "";
  this.focusRightPadding =
    options.focusRightPadding ?? options.rightPadding ?? "";
  this.disabledLeftPadding =
    options.disabledLeftPadding ?? options.leftPadding ?? "";
  this.disabledRightPadding =
    options.disabledRightPadding ?? options.rightPadding ?? "";
  this.submittedLeftPadding =
    options.submittedLeftPadding ?? options.leftPadding ?? "";
  this.submittedRightPadding =
    options.submittedRightPadding ?? options.rightPadding ?? "";
  this.turnedOnBlurLeftPadding =
    options.turnedOnBlurLeftPadding ??
    options.turnedOnLeftPadding ??
    this.blurLeftPadding;
  this.turnedOnBlurRightPadding =
    options.turnedOnBlurRightPadding ??
    options.turnedOnRightPadding ??
    this.blurRightPadding;
  this.turnedOffBlurLeftPadding =
    options.turnedOffBlurLeftPadding ??
    options.turnedOffLeftPadding ??
    this.blurLeftPadding;
  this.turnedOffBlurRightPadding =
    options.turnedOffBlurRightPadding ??
    options.turnedOffRightPadding ??
    this.blurRightPadding;
  this.turnedOnFocusLeftPadding =
    options.turnedOnFocusLeftPadding ??
    options.turnedOnLeftPadding ??
    this.focusLeftPadding;
  this.turnedOnFocusRightPadding =
    options.turnedOnFocusRightPadding ??
    options.turnedOnRightPadding ??
    this.focusRightPadding;
  this.turnedOffFocusLeftPadding =
    options.turnedOffFocusLeftPadding ??
    options.turnedOffLeftPadding ??
    this.focusLeftPadding;
  this.turnedOffFocusRightPadding =
    options.turnedOffFocusRightPadding ??
    options.turnedOffRightPadding ??
    this.focusRightPadding;
  ...
  if (options.buttonKeyBindings) {
    this.buttonKeyBindings = options.buttonKeyBindings;
  }
  if (options.buttonActionKeyBindings) {
    this.buttonActionKeyBindings = options.buttonActionKeyBindings;
  }
  if (options.toggleButtonKeyBindings) {
    this.toggleButtonKeyBindings = options.toggleButtonKeyBindings;
  }
  if (options.toggleButtonActionKeyBindings) {
    this.toggleButtonActionKeyBindings = options.toggleButtonActionKeyBindings;
  }
...
BaseMenu.prototype.focusValue = function (
  itemValue,
  focusType,
  forceInitPage = false,
) {
...
BaseMenu.prototype.onWheel = function (data) {
  if (data.yDirection < 0) {
    this.previousPage("backCycle");
  } else if (data.yDirection > 0) {
    this.nextPage("cycle");
  }
};

BaseMenu.prototype.onFocus = function (focus, type) {
  if (type === "cycle" || type === "backCycle") {
    return;
  }
  ...
  switch (button.internalRole) {
    ...
    default:
      if (
        ...
        this.hasSubmenu &&
        button.def.items &&
        this.submenuOptions.openOn === "parentFocus"
      ) {
        this.openSubmenu(button.value, button);
      }
...
BaseMenu.prototype.onSubmenuSubmit = function (
  buttonValue,
  action,
  submenu,
  button,
...
BaseMenu.prototype.openSubmenu = function (itemValue, button = null) {
  var x,
    ...
    height,
    itemDef = button
      ? this.itemsDef.find((it) => it === button.def)
      : this.itemsDef.find((it) => it.value === itemValue);

  if (!itemDef || !itemDef.items || !itemDef.items.length) {
    return;
  }

  if (this.submenu) {
    if (this.submenu.def === itemDef) {
      return;
    }
    this.closeSubmenu();
  }
  ...
  this.submenu = new this.constructor(
    Object.assign({}, this.submenuOptions, {
      internal: true,
      parent: this,
      isSubmenu: true,
      def: itemDef,
...
BaseMenu.prototype.closeSubmenu = function () {
  ...
  if (this.submenuOptions.hideParent) {
    this.children.forEach((e) => (e.hidden = false));
  }
...
userActions.character = userActions.specialKey = function (key) {
...
userActions.next = function () {
  ...
  if (
    this.focusChild === this.children[this.children.length - 1] &&
    this.maxPage &&
    this.page < this.maxPage
  ) {
    this.nextPage("cycle");
...
userActions.parentMenu = function () {
  if (this.isSubmenu) {
    ...
    if (this.parent.submenuOptions.hideParent) {
