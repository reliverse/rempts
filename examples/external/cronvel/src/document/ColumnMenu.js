...
ColumnMenu.prototype.initChildren = function (noInitPage = false) {
    ...
    this.previousPageDef.width =
        this.buttonPaddingWidth +
            Element.computeContentWidth(this.previousPageDef.content, this.previousPageDef.contentHasMarkup);
    ...
    this.nextPageDef.width =
        this.buttonPaddingWidth +
            Element.computeContentWidth(this.nextPageDef.content, this.nextPageDef.contentHasMarkup);
    ...
    if (this.masterDef) {
        ...
        this.masterDef.width =
            this.buttonPaddingWidth +
                Element.computeContentWidth(this.masterDef.buttonContent, this.masterDef.contentHasMarkup);
    }
    ...
    this.itemsDef.forEach((def, index) => {
        ...
        var contentWidth = Element.computeContentWidth(def.content, def.contentHasMarkup), buttonHeight = 1, isLastItem = index === this.itemsDef.length - 1, 
        // height without previous/next buttons:
        pageMaxInnerHeight = Math.max(1, !isLastItem ? this.maxHeight - 1 : this.maxHeight), overflow = this.buttonPaddingWidth + contentWidth - this.outputWidth;
        if (overflow > 0) {
            if (this.multiLineItems) {
                ...
                def.buttonContent =
                    Element.truncateContent(def.content, contentWidth - overflow - ellipsisWidth, def.contentHasMarkup) + this.contentEllipsis;
        ...
        if (index && pageHeight + buttonHeight > pageMaxInnerHeight) {
            ...
            if (pageHeight + buttonHeight > pageMaxInnerHeight && buttonHeight > 1) {
                ...
                def.buttonContent[buttonHeight - 1] =
                    Element.truncateContent(def.buttonContent[buttonHeight - 1].trimRight(), contentWidth - ellipsisWidth, def.contentHasMarkup) + this.contentEllipsis;
            }
        }
    ...
    if (this.separatorDef) {
        ...
        if (this.separatorDef.contentRepeat &&
            this.separatorDef.width < this.buttonsMaxWidth - this.buttonPaddingWidth) {
            this.separatorDef.content = this.separatorDef.content.repeat(Math.floor((this.buttonsMaxWidth - this.buttonPaddingWidth) /
