...
RowMenu.prototype.initChildren = function (noInitPage = false) {
    ...
    this.itemsDef.forEach((def, index) => {
        ...
        var contentWidth = Element.computeContentWidth(def.content, def.contentHasMarkup), isLastItem = index === this.itemsDef.length - 1;
        def.width = contentWidth + this.buttonPaddingWidth + this.buttonSymbolWidth;
        var overflow = pageWidth +
            def.width +
    ...
    this.pageItemsDef.forEach((pageDef, index) => {
        ...
        pageDef.justifyWidth = Math.max(0, this.justify
            ? (this.outputWidth -
                this.leftMargin -
                pageDef.buttonsAndSeparatorsWidth) /
                (pageDef.length - 1)
...
RowMenu.prototype.initPage = function (page = this.page) {
    var pageDef = this.pageItemsDef[page], justifyWidthError = 0, buttonOffsetX = this.leftMargin, buttonOffsetY = 0;
    if (!pageDef) {
        return;
    }
