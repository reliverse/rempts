...
*/
export default function gridMenu(menuItems_, options, callback) {
      if (arguments.length < 1) {
        throw new Error(
          "[terminal] gridMenu() needs at least an array of menuItems argument",
        );
      }

      if (!Array.isArray(menuItems_) || !menuItems_.length) {
        throw new TypeError(
          "[terminal] gridMenu(): argument #0 should be a non-empty array",
        );
      }
      ...
      if (options.rightPadding === undefined) {
        options.rightPadding = " ";
      }
      if (options.selectedRightPadding === undefined) {
      ...
      var redrawCursor = () => {
        this.moveTo(
          1 + menuItems[selectedIndex].offsetX,
          start.y + menuItems[selectedIndex].offsetY,
        );
      };
