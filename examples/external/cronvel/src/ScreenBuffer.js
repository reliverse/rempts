...
function ScreenBuffer(options = {}) {
    ...
    this.width =
        Math.floor(options.width) || (options.dst ? options.dst.width : 1);
    this.height =
        Math.floor(options.height) || (options.dst ? options.dst.height : 1);
    this.x =
        options.x !== undefined
            ? options.x
            : options.dst && options.dst instanceof termkit.Terminal
                ? 1
...
ScreenBuffer.createFromString = function (options, data) {
    ...
    if (typeof data !== "string") {
        if (!data.toString) {
            throw new Error("[terminal] ScreenBuffer.createFromDataString(): argument #1 should be a string or provide a .toString() method.");
        }
        data = data.toString();
    }
    ...
    attr =
        options.attr !== undefined
            ? options.attr
            : ScreenBuffer.prototype.DEFAULT_ATTR;
    if (attr && typeof attr === "object" && !attr.BYTES_PER_ELEMENT) {
        attr = ScreenBuffer.object2attr(attr);
    }
    attrTrans = attr;
    if (options.transparencyChar) {
        if (!options.transparencyType) {
            attrTrans |= TRANSPARENCY;
        }
        else {
            attrTrans |= options.transparencyType & TRANSPARENCY;
        }
    }
...
ScreenBuffer.prototype.fill = function (options) {
    var i, attr, char, start, end, region, srcRect, toRect, clearBuffer = this.CLEAR_BUFFER, buffer = this.buffer;
    if (options && typeof options === "object") {
        if (options.char || options.attr) {
            ...
            // Write the character
            char =
                options.char && typeof options.char === "string" ? options.char : " ";
        ...
        start = options.start ? Math.floor(options.start / this.ITEM_SIZE) : 0;
            ...
            ? Math.floor(options.end / this.ITEM_SIZE)
            : buffer.length / this.ITEM_SIZE;
...
ScreenBuffer.prototype.put = function (options, str, ...args) {
    ...
    if (options.resumeAttr !== undefined) {
    ...
    var processRaw = (part) => {
        ...
        for (let i = 0; i < iMax; i++) {
            ...
            if (y >= ymin && y <= ymax) {
                if (isFullWidth) {
                    if (x + isFullWidth * dx >= xmin &&
                        x + isFullWidth * (dx || 1) <= xmax) {
                        ...
                        if (this.hasTrailingFullWidth(this.buffer, offset) && x) {
                            this.removeFullWidth(this.buffer, offset - this.ITEM_SIZE);
                        }
                        // Write the attributes
                        ...
                        offset += this.ITEM_SIZE;
                        // Check if we are writing on a fullwidth char
                        if (this.hasLeadingFullWidth(this.buffer, offset) && x < xmax) {
                            this.removeFullWidth(this.buffer, offset + this.ITEM_SIZE);
                        }
                        // Write the attributes
                    ...
                    if (x >= xmin && x <= xmax) {
                        ...
                        else if (this.hasTrailingFullWidth(this.buffer, offset) && x) {
                            this.removeFullWidth(this.buffer, offset - this.ITEM_SIZE);
                        }
                        // Write the attributes
            ...
            if (!inBounds && lastValidOffset >= 0 && options.clipChar) {
                // Check if we are writing on a fullwidth char
                if (this.hasLeadingFullWidth(this.buffer, lastValidOffset) &&
                    x < xmax) {
                    this.removeFullWidth(this.buffer, lastValidOffset + this.ITEM_SIZE);
                }
                else if (this.hasTrailingFullWidth(this.buffer, lastValidOffset) &&
                    x) {
                    this.removeFullWidth(this.buffer, lastValidOffset - this.ITEM_SIZE);
                }
                // Write the character
    ...
    if (!options.markup) {
        ...
        for (let part of parts) {
            ...
            if (attrObject.defaultColor && attrObject.color) {
                delete attrObject.defaultColor;
            }
            if (attrObject.bgDefaultColor && attrObject.bgColor) {
                delete attrObject.bgDefaultColor;
            }
            attr = this.object2attr(attrObject, undefined, legacyColor);
            if (part.text) {
                if (processRaw(part.text)) {
                    break;
                }
            }
        }
...
ScreenBuffer.prototype.resize = function (fromRect) {
    ...
    var offsetX = -fromRect.xmin, offsetY = -fromRect.ymin;
...
ScreenBuffer.prototype.draw = function (options) {
    ...
    var tr = {
        ...
        dstClipRect: options.dstClipRect
            ? new Rect(options.dstClipRect)
            : undefined,
        srcClipRect: options.srcClipRect
            ? new Rect(options.srcClipRect)
            : undefined,
        delta: options.delta,
...
ScreenBuffer.prototype.drawCursor = function (options) {
    ...
    if (dst instanceof ScreenBuffer) {
        ...
        if (this.ch) {
            if (this.ch !== this.lastCh) {
                dst.hideCursor();
            }
        }
        else {
            if (this.ch !== this.lastCh) {
                dst.hideCursor(false);
            }
            dst.moveTo(Math.max(1, Math.min(this.cx + this.x, dst.width)), Math.max(1, Math.min(this.cy + this.y, dst.height)));
        }
        this.lastCh = this.ch;
    }
};
...
ScreenBuffer.prototype.blitterCellBlendingIterator = function (p) {
    var attr = this.readAttr(p.context.srcBuffer, p.srcStart), blending = attr & TRANSPARENCY;
    ...
    if (!(blending & FG_TRANSPARENCY)) {
        ...
        p.context.srcBuffer.copy(p.context.dstBuffer, p.dstStart + 3, p.srcStart + 3, p.srcStart + 4);
    }
    if (!(blending & BG_TRANSPARENCY)) {
        // Copy source background color
        p.context.srcBuffer.copy(p.context.dstBuffer, p.dstStart + 2, p.srcStart + 2, p.srcStart + 3);
    }
    if (!(blending & STYLE_TRANSPARENCY)) {
        // Copy source style
        p.context.srcBuffer.copy(p.context.dstBuffer, p.dstStart + 1, p.srcStart + 1, p.srcStart + 2);
    }
    if (!(blending & CHAR_TRANSPARENCY)) {
        if (p.endOfBlitLine && attr & LEADING_FULLWIDTH) {
            ...
            p.context.srcBuffer.copy(p.context.dstBuffer, p.dstStart + this.ATTR_SIZE, p.srcStart + this.ATTR_SIZE, p.srcEnd);
        }
    }
};
// Shared
ScreenBuffer.prototype.terminalBlitter = function (p) {
    ...
    if (p.inline) {
        tr.dstRect.setSize({ height: Infinity });
    }
    if (p.delta && !p.inline) {
    ...
    if (context.sequence.length) {
        context.rawTerm(context.sequence);
        context.writes++;
    }
...
ScreenBuffer.prototype.terminalBlitterLineIterator = function (p) {
    ...
    for (offset = p.srcStart; offset < p.srcEnd; offset += this.ITEM_SIZE) {
        ...
        if ((attr & ATTR_MASK) !== (p.context.lastAttr & ATTR_MASK)) {
            p.context.sequence +=
                p.context.lastAttr === null || !p.context.deltaEscapeSequence
...
ScreenBuffer.prototype.terminalBlitterCellIterator = function (p) {
    ...
    if (p.context.srcLastBuffer) {
        if (attr === this.readAttr(p.context.srcLastBuffer, p.srcStart) &&
            this.readChar(p.context.srcBuffer, p.srcStart) ===
                this.readChar(p.context.srcLastBuffer, p.srcStart)) {
            return;
        }
...
ScreenBuffer.fromNdarrayImage = function (pixels, options) {
    ...
    var x, xMax = pixels.shape[0], y, yMax = Math.ceil(pixels.shape[1] / 2), hasAlpha = pixels.shape[2] === 4, maxRegister = term.support["256colors"] ? 255 : 15, fgColor, bgColor, cache = {};
    ...
    for (x = 0; x < xMax; x++) {
        for (y = 0; y < yMax; y++) {
            fgColor = term.registerForRgbCache(cache, pixels.get(x, y * 2, 0), pixels.get(x, y * 2, 1), pixels.get(x, y * 2, 2), 0, maxRegister, 1);
            if (y * 2 + 1 < pixels.shape[1]) {
                bgColor = term.registerForRgbCache(cache, pixels.get(x, y * 2 + 1, 0), pixels.get(x, y * 2 + 1, 1), pixels.get(x, y * 2 + 1, 2), 0, maxRegister, 1);
                image.put({
                    x: x,
                    y: y,
                    attr: {
...
ScreenBuffer.prototype.dump = function () {
    ...
    for (y = 0; y < this.height; y++) {
        for (x = 0; x < this.width; x++) {
            ...
            str += string.format("%x%x%x%x ", this.buffer.readUInt8(offset), this.buffer.readUInt8(offset + 1), this.buffer.readUInt8(offset + 2), this.buffer.readUInt8(offset + 3));
        }
        str += "\n";
    }
    return str;
...
ScreenBuffer.prototype.readChar = function (buffer, at) {
    ...
    else if (buffer[at] < 0xc0) {
        return "\x00";
    } // We are in a middle of an unicode multibyte sequence... something was wrong...
    else if (buffer[at] < 0xe0) {
        bytes = 2;
    }
    else if (buffer[at] < 0xf0) {
        bytes = 3;
    }
    else if (buffer[at] < 0xf8) {
        bytes = 4;
    }
    else if (buffer[at] < 0xfc) {
        bytes = 5;
    }
    else {
        bytes = 6;
    }
    if (bytes > this.CHAR_SIZE) {
        return "\x00";
    }
...
ScreenBuffer.prototype.generateEscapeSequence = function (term, attr) {
    ...
    var esc = term.optimized.styleReset +
        (attr & FG_DEFAULT_COLOR
            ...
            : palette.escape[attr & 255]) +
        (attr & BG_DEFAULT_COLOR
            ? term.optimized.bgDefaultColor
            : palette.bgEscape[(attr >>> 8) & 255]);
    // Style part
    if (attr & BOLD) {
        esc += term.optimized.bold;
    }
    if (attr & DIM) {
        esc += term.optimized.dim;
    }
    if (attr & ITALIC) {
        esc += term.optimized.italic;
    }
    if (attr & UNDERLINE) {
        esc += term.optimized.underline;
    }
    if (attr & BLINK) {
        esc += term.optimized.blink;
    }
    if (attr & INVERSE) {
        esc += term.optimized.inverse;
    }
    if (attr & HIDDEN) {
        esc += term.optimized.hidden;
    }
    if (attr & STRIKE) {
        esc += term.optimized.strike;
    }
    return esc;
};
...
ScreenBuffer.prototype.generateDeltaEscapeSequence = function (term, attr, lastAttr) {
    ...
    if (attr & FG_DEFAULT_COLOR) {
        if (!(lastAttr & FG_DEFAULT_COLOR)) {
            esc += term.optimized.defaultColor;
        }
    }
    ...
    if (attr & BG_DEFAULT_COLOR) {
        if (!(lastAttr & BG_DEFAULT_COLOR)) {
            esc += term.optimized.bgDefaultColor;
        }
    }
    ...
    if ((attr & STYLE_MASK) !== (lastAttr & STYLE_MASK)) {
        ...
        if ((attr & BOLD_DIM) !== (lastAttr & BOLD_DIM)) {
            if ((lastAttr & BOLD && !(attr & BOLD)) ||
                ...
                if (attr & BOLD && !(lastAttr & BOLD)) {
                    esc += term.optimized.bold;
                }
                if (attr & DIM && !(lastAttr & DIM)) {
                    esc += term.optimized.dim;
                }
            }
        }
        ...
        if ((attr & UNDERLINE) !== (lastAttr & UNDERLINE)) {
            esc +=
                attr & UNDERLINE
                    ? term.optimized.underline
                    : term.optimized.noUnderline;
        }
...
ScreenBuffer.prototype.vScroll = function (lineOffset, attr, ymin, ymax, terminalScrolling) {
    ...
    if (ymin === undefined || ymin === null) {
        ...
        if (ymax > this.height - 1) {
            ymax = this.height - 1;
        }
    }
    var scrollOffset = lineOffset * this.width * this.ITEM_SIZE, startOffset = ymin * this.width * this.ITEM_SIZE, endOffset = (ymax + 1) * this.width * this.ITEM_SIZE;
    if (scrollOffset > 0) {
    ...
    if (terminalScrolling && this.dst instanceof termkit.Terminal) {
        ...
        if (lineOffset > 0) {
            this.dst.scrollDown(lineOffset);
        }
        else {
            this.dst.scrollUp(-lineOffset);
        }
        this.dst.resetScrollingRegion();
    }
};
ScreenBuffer.prototype.copyRegion = function (from, to, isMove, attr) {
    var bufferRect = new Rect(this), fromRect = new Rect(from), toRect = new Rect(to);
...
ScreenBuffer.attr2object = function (attr) {
    ...
    // Blending part
    if (attr & FG_TRANSPARENCY) {
        object.fgTransparency = true;
    }
    if (attr & BG_TRANSPARENCY) {
        object.bgTransparency = true;
    }
    if (attr & STYLE_TRANSPARENCY) {
        object.styleTransparency = true;
    }
    if (attr & CHAR_TRANSPARENCY) {
        object.charTransparency = true;
    }
    if ((attr & TRANSPARENCY) === TRANSPARENCY) {
...
ScreenBuffer.object2attr = function (object, colorNameToIndex, legacyColor = false) {
    ...
    if (typeof object.color === "string") {
        if (object.color === "default") {
            ...
            object.color = colorNameToIndex(object.color);
        }
    }
    if (typeof object.color !== "number" ||
        ...
        if (legacyColor && object.color <= 15) {
            if (brightFg && object.color <= 7) {
                object.color += 8;
            }
            else if (!brightFg && object.color >= 8) {
                object.color -= 8;
            }
        }
    }
    ...
    if (typeof object.bgColor === "string") {
        if (object.bgColor === "default") {
            object.bgColor = 0;
            object.bgDefaultColor = true;
        }
        else {
            object.bgColor = colorNameToIndex(object.bgColor);
        }
    }
    if (typeof object.bgColor !== "number" ||
        object.bgColor < 0 ||
        object.bgColor > 255) {
        object.bgColor = 0;
        object.bgDefaultColor = true;
        ...
        if (legacyColor && object.bgColor <= 15) {
            if (brightBg && object.bgColor <= 7) {
                object.bgColor += 8;
            }
            else if (!brightBg && object.bgColor >= 8) {
                object.bgColor -= 8;
            }
        }
    }
    ...
    if (object.fgTransparency) {
        attr |= FG_TRANSPARENCY;
    }
    if (object.bgTransparency) {
        attr |= BG_TRANSPARENCY;
    }
    if (object.styleTransparency) {
        attr |= STYLE_TRANSPARENCY;
    }
    if (object.charTransparency) {
        attr |= CHAR_TRANSPARENCY;
    }
    return attr;
};
...
ScreenBuffer.attrAndObject = function (attr, object, colorNameToIndex) {
    ...
    if (object.bgDefaultColor || object.bgColor === "default") {
        ...
        attr =
            attr -
                (((attr >>> 8) & 255) << 8) +
                (colorNameToIndex(object.bgColor) << 8);
        if (attr & BG_DEFAULT_COLOR) {
            attr ^= BG_DEFAULT_COLOR;
        }
    }
    else if (typeof object.bgColor === "number" &&
        object.bgColor >= 0 &&
        object.bgColor <= 255) {
    ...
    else if (object.bold === false) {
        attr &= ~BOLD;
    }
    if (object.dim === true) {
        attr |= DIM;
    }
    else if (object.dim === false) {
        attr &= ~DIM;
    }
    if (object.italic === true) {
        attr |= ITALIC;
    }
    else if (object.italic === false) {
        attr &= ~ITALIC;
    }
    if (object.underline === true) {
        attr |= UNDERLINE;
    }
    else if (object.underline === false) {
        attr &= ~UNDERLINE;
    }
    if (object.blink === true) {
        attr |= BLINK;
    }
    else if (object.blink === false) {
        attr &= ~BLINK;
    }
    if (object.inverse === true) {
        attr |= INVERSE;
    }
    else if (object.inverse === false) {
        attr &= ~INVERSE;
    }
    ...
    // Blending part
    if (object.transparency === true) {
        attr |= TRANSPARENCY;
    }
    else if (object.transparency === false) {
        attr &= ~TRANSPARENCY;
    }
    if (object.fgTransparency === true) {
        attr |= FG_TRANSPARENCY;
    }
    else if (object.fgTransparency === false) {
        attr &= ~FG_TRANSPARENCY;
    }
    if (object.bgTransparency === true) {
        attr |= BG_TRANSPARENCY;
    }
    else if (object.bgTransparency === false) {
        attr &= ~BG_TRANSPARENCY;
    }
    if (object.styleTransparency === true) {
        attr |= STYLE_TRANSPARENCY;
    }
    else if (object.styleTransparency === false) {
        attr &= ~STYLE_TRANSPARENCY;
    }
    if (object.charTransparency === true) {
        attr |= CHAR_TRANSPARENCY;
    }
    else if (object.charTransparency === false) {
        attr &= ~CHAR_TRANSPARENCY;
    }
    return attr;
};
...
ScreenBuffer.prototype.ITEM_SIZE =
    ScreenBuffer.prototype.ATTR_SIZE + ScreenBuffer.prototype.CHAR_SIZE;
ScreenBuffer.DEFAULT_ATTR = // <- used by TextBuffer
    ScreenBuffer.prototype.DEFAULT_ATTR = ScreenBuffer.object2attr({
        defaultColor: true,
        bgDefaultColor: true,
    });
ScreenBuffer.prototype.CLEAR_ATTR = ScreenBuffer.object2attr({
    defaultColor: true,
    bgDefaultColor: true,
    transparency: true,
});
...
ScreenBuffer.loadSyncV1 = function (filepath) {
    ...
    if (content.length !== size) {
        throw new Error("Bad file size: this is not a ScreenBuffer file");
    }
...
ScreenBuffer.loadSyncV2 = function (filepath) {
    ...
    // Check bitsPerColor
    if (header.bitsPerColor &&
        header.bitsPerColor !== ScreenBuffer.prototype.bitsPerColor) {
        throw new Error("Bad Bits Per Color: " +
            header.bitsPerColor +
            " (should be " +
            ScreenBuffer.prototype.bitsPerColor +
            ")");
    }
    // Bad size?
    if (content.length !==
        i + 1 + header.width * header.height * ScreenBuffer.prototype.ITEM_SIZE) {
