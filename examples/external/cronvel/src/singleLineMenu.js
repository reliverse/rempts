...
*/
export default function singleLineMenu(menuItems_, options, callback) {
    ...
    if (options.nextPageHint === undefined) {
        options.nextPageHint = " » ";
    }
    if (options.previousPageHint === undefined) {
        options.previousPageHint = " « ";
    }
    if (!options.style) {
        options.style = this;
    }
    ...
    var selectedIndexInPage = (options.selectedIndex =
        options.selectedIndex || 0);
    var start = {}, selectedPage = 0, finished = false, menuPages = [], alreadyCleanedUp = false;
    // Width
    var nextPageHintWidth = stringWidth(options.nextPageHint), previousPageHintWidth = stringWidth(options.previousPageHint), separatorWidth = stringWidth(options.separator);
    var computePages = () => {
        ...
        for (i = 0; i < menuItems.length; i++) {
            ...
            if (nextEndX > xMax) {
                if (firstItem) {
                    itemWidth = xMax - endX;
                    displayText =
                        termkit.truncateString(menuItems[i], itemWidth - 1) + "…";
    ...
    var onKey = (key, trash, data) => {
        ...
        switch (keyBindings[key]) {
            ...
            case "cycleNext":
                if (selectedPage === menuPages.length - 1 &&
                    selectedIndexInPage === page.length - 1) {
                    selectedPage = 0;
                    selectedIndexInPage = 0;
                    changed = true;
                }
    ...
    var onMouse = (name, data) => {
        ...
        if (name === "MOUSE_LEFT_BUTTON_PRESSED") {
            if (selectedPage > 0 &&
                data.x >= 1 &&
                data.x < 1 + previousPageHintWidth) {
                selectedPage--;
                selectedIndexInPage = 0;
                redraw();
                emitHighlight();
            ...
            nextButtonX =
                page[page.length - 1].x + page[page.length - 1].displayTextWidth;
            if (selectedPage < menuPages.length - 1 &&
                data.x >= nextButtonX &&
