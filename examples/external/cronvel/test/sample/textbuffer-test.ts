...
term
  .moveTo(1, 1)
  .bgWhite.blue("Welcome to Neon!   ")
  .bgWhite.green("Ctrl-C to quit");
term.grabInput();
...
if (isHd) {
  attrs = [
    ...
    {
      color: { r: 200, g: 50, b: 50 },
      bgColor: { r: 0, g: 0, b: 70 },
      bold: true,
      italic: true,
    },
    {
      color: { r: 30, g: 170, b: 170 },
      bgColor: { r: 70, g: 0, b: 0 },
    },
  ];
...
term.on("key", (key, matches, data) => {
  ...
  if (data.isCharacter) {
    ...
    switch (key) {
      case "CTRL_K":
        term.saveCursor();
        term.moveTo.styleReset.eraseLine.eraseDisplayBelow(
          1,
          12,
          "Current: %Y\n%Y",
          tbuf.getText(),
          tbuf.buffer.map((line) => line.map((cell) => JSON.stringify(cell))),
        );
        term.restoreCursor();
        break;
      case "CTRL_S":
