...
async function testTerminal(t) {
    ...
    term("Config file: %s\n", t.termconfigFile);
    term("\n");
    term("Support for delta escape sequence: " +
        (t.support.deltaEscapeSequence ? "^GOK^:\n" : "^RNO^:\n"));
    term("Support for 256 colors: " +
        (t.support["256colors"] ? "^GOK^:\n" : "^RNO^:\n"));
    term("Support for true colors: " +
        (t.support.trueColor ? "^GOK^:\n" : "^RNO^:\n"));
    try {
    ...
    term("Issue #116 CURSOR_LOCATION keymap: " +
        (t.keymap.CURSOR_LOCATION && typeof t.keymap.CURSOR_LOCATION === "object"
            ? "^GOK^:\n"
            : "^RNO^:\n"));
    term("Issue #116 cursorLocation handler: " +
        (typeof t.handler.cursorLocation === "function"
            ? "^GOK^:\n"
            : "^RNO^:\n"));
    term("\n");
}
