...
function TextBuffer(options = {}) {
  this.ScreenBuffer =
    ...
    (options.dst && options.dst.constructor) ||
  ...
  this.dstClipRect = options.dstClipRect
    ? new termkit.Rect(options.dstClipRect)
    : null;
...
TextBuffer.prototype.removeTrailingSpaces = function (
  y = this.cy,
  x = null,
  dry = false,
) {
...
TextBuffer.prototype.setText = function (text, hasMarkup, baseAttr) {
  ...
  var legacyColor = false,
    parser = null;

  switch (hasMarkup) {
    case "ansi":
      parser = string.ansi.parse;
      break;
    case "legacyAnsi":
      parser = string.ansi.parse;
      legacyColor = true;
      break;
    case true:
      parser = this.parseMarkup;
      break;
  }

  if (baseAttr === undefined) {
    baseAttr = this.defaultAttr;
  }
  if (typeof baseAttr === "object") {
    baseAttr = this.object2attr(baseAttr);
  }
  ...
  text.split(/(?<=\n)/g).forEach((line) => {
    ...
    this.buffer[index] = this.lineToCells(
      line,
      parser,
      baseAttr,
      0,
...
TextBuffer.prototype.lineToCells = function (
  line,
  parser,
  baseAttr,
  offset = 0,
  ...
  const defaultAttrObject = this.ScreenBuffer.attr2object(
    this.ScreenBuffer.DEFAULT_ATTR,
  );
...
TextBuffer.prototype.setHidden = function (value) {
  this.hidden =
    typeof value === "string" && value.length
      ? value[0]
      : value
        ? termkit.spChars.password
        : false;
};

TextBuffer.prototype.getHidden = function () {
  return this.hidden;
};
...
TextBuffer.prototype.getVoidTextBuffer = function () {
  return this.voidTextBuffer;
};
...
TextBuffer.prototype.reTabLine = function (startAt = 0, y = this.cy) {
  ...
  for (index = startAt; index < length; index++) {
    ...
    if (cell.char === "\t") {
      ...
      // Skip input filler
      while (index + 1 < length && input[index + 1].filler) {
        index++;
      }
    } else {
      output.push(cell);
...
TextBuffer.prototype.wrapLine = function (
  startY = this.cy,
  width = this.lineWrapWidth,
  ...
  if (
    ...
    (line.length &&
      ...
      (!previousLine ||
        !previousLine.length ||
        previousLine[previousLine.length - 1].char === "\n"))
  ) {
  ...
  while (unifiedLine.length) {
    ...
    if (unifiedLine[x].char === " ") {
      ...
      while (x < unifiedLine.length && unifiedLine[x].char === " ") {
        x++;
      }
      ...
      if (
        x === lineWidth + 1 &&
        FORBIDDEN_SPLIT.has(unifiedLine[x].char) &&
        unifiedLine[lineWidth - 1].char !== " "
      ) {
    ...
    if (!found) {
      ...
      while (
        x >= 0 &&
        (unifiedLine[x].char !== " " ||
          (FORBIDDEN_SPLIT.has(lastChar) &&
            x > 0 &&
            unifiedLine[x - 1].char !== " "))
      ) {
      ...
      if (x < 0) {
        x = lineWidth;
      } // No space found, cut at the lineWidth
      else {
        x++;
      } // Cut just after the space
    }
  ...
  if (checkCursor) {
    ...
    if (
      this.cx &&
      this.buffer[this.cy] &&
      this.buffer[this.cy][this.cx - 1] &&
      this.buffer[this.cy][this.cx - 1].char === "\n"
    ) {
      this.cy++;
      this.cx = 0;
...
TextBuffer.prototype.wrapAllLines = function (
  width = this.lineWrapWidth,
  wordWrap = this.wordWrap,
) {
  var y = 0;
...
// Probably DEPRECATED
TextBuffer.prototype.wordWrapLine = function (
  startY = this.cy,
  width = this.lineWrapWidth,
) {
...
TextBuffer.prototype.setAttrCodeAt = function (attr, x, y) {
  ...
  if (!this.buffer[y][x]) {
    this.buffer[y][x] = new Cell(" ", 1, attr);
  } else {
...
TextBuffer.prototype.setAttrCodeRegion = function (
  attr,
  region = WHOLE_BUFFER_REGION,
) {
  ...
  for (y = region.ymin; y <= ymax; y++) {
    ...
    xmax =
      y === region.ymax
        ? Math.min(region.xmax, this.buffer[y].length - 1)
...
TextBuffer.prototype.setSelectionRegion = function (region) {
  ...
  if (region.xmin !== undefined && region.ymin !== undefined) {
    ...
    this.selectionRegion.cellMin =
      this.buffer[region.ymin]?.[region.xmin] ?? null;
  }

  if (region.xmax !== undefined && region.ymax !== undefined) {
    this.selectionRegion.xmax = region.xmax;
    this.selectionRegion.ymax = region.ymax;
    this.selectionRegion.cellMax =
      this.buffer[region.ymax]?.[region.xmax] ?? null;
  }
};
...
TextBuffer.prototype.getRegionText = function (region, structured = false) {
  ...
  if (
    !region ||
    region.xmin === undefined ||
    region.ymin === undefined ||
    region.xmax === undefined ||
    region.ymax === undefined
  ) {
    return;
  }
...
TextBuffer.prototype.deleteRegion = function (region, getDeleted = false) {
  ...
  if (
    ...
    (!currentLine.length || currentLine[currentLine.length - 1].char !== "\n")
  ) {
...
TextBuffer.prototype.getMisc = function () {
  if (!this.buffer[this.cy] || !this.buffer[this.cy][this.cx]) {
    return;
  }
  if (!this.buffer[this.cy][this.cx].misc) {
    this.buffer[this.cy][this.cx].misc = {};
  }
...
TextBuffer.prototype.getMiscAt = function (x, y) {
  ...
  if (!this.buffer[y][x].misc) {
    this.buffer[y][x].misc = {};
  }
...
TextBuffer.prototype.moveTo = function (x, y) {
  this.cx = x >= 0 ? x : 0;
  this.cy = y >= 0 ? y : 0;
  if (this.forceInBound) {
    this.moveInBound(true);
  }
  this.moveToLeadingFullWidth();
};

TextBuffer.prototype.move = function (x, y) {
  this.moveTo(this.cx + x, this.cy + y);
};
TextBuffer.prototype.moveToColumn = function (x) {
  this.moveTo(x, this.cy);
};
TextBuffer.prototype.moveToLine = TextBuffer.prototype.moveToRow = function (
  y,
) {
  this.moveTo(this.cx, y);
};
...
TextBuffer.prototype.moveForward = function (testFn, justSkipFiller) {
  ...
  if (
    justSkipFiller &&
    (!currentLine || !currentLine[this.cx] || !currentLine[this.cx].filler)
  ) {
    return;
  }
  ...
  for (;;) {
    if (
      !currentLine ||
      this.cx + 1 > currentLine.length ||
      (this.cx < currentLine.length && currentLine[this.cx].char === "\n")
    ) {
...
TextBuffer.prototype.moveToStartOfLine = function () {
  this.cx = 0;
};
...
TextBuffer.prototype.moveToStartOfBuffer = function () {
  this.cx = this.cy = 0;
};
...
TextBuffer.prototype.moveInBound = function (ignoreCx) {
  ...
  if (!currentLine) {
    ...
    if (this.cx > currentLine.length - 1) {
      this.cx = currentLine.length - 1;
    }
...
TextBuffer.prototype.inlineInsert = function (
  text,
  parser,
  attr,
  legacyColor = false,
...
TextBuffer.prototype.delete = function (count, getDeleted = false) {
  ...
  if (
    this.buffer[this.cy] &&
    this.buffer[this.cy][this.cx] &&
    this.buffer[this.cy][this.cx].filler
  ) {
  ...
  while (count > 0) {
    ...
    if (currentLine) {
      ...
      if (currentLine[this.cx] && currentLine[this.cx].char !== "\n") {
        ...
        inlineCount = Math.min(
          count + fillerCount,
          currentLine.length - hasNL - this.cx,
        );
...
TextBuffer.prototype.backDelete = function (count, getDeleted = false) {
  ...
  if (
    ...
    this.cx &&
    this.buffer[this.cy][this.cx - 1] &&
    this.buffer[this.cy][this.cx - 1].filler
  ) {
    this.moveBackward(undefined, true); // just skip filler char
    ...
    if (currentLine) {
      ...
      if (inlineCount > 0) {
        ...
        if (getDeleted) {
          removedCells = removedCells.filter((cell) => !cell.filler);
          deleted.string =
            removedCells.map((cell) => cell.char).join("") + deleted.string;
          deleted.count += removedCells.length;
        }
        this.cx -= inlineCount;
      }
...
TextBuffer.prototype.newLine = function (internalCall) {
  ...
  if (!internalCall && this.forceInBound) {
    this.moveInBound();
  }

  if (this.buffer[this.cy] === undefined) {
    this.buffer[this.cy] = [];
  }
...
TextBuffer.prototype.findNext = function (
  searchString,
  startPosition,
  reverse,
) {
  ...
  index = reverse
    ? text.lastIndexOf(searchString, startPosition)
...
TextBuffer.prototype.draw = function (options = {}) {
  ...
  var tr = {
    ...
    srcClipRect: options.srcClipRect
      ? new termkit.Rect(options.srcClipRect)
      : undefined,
...
TextBuffer.prototype.drawCursor = function (options) {
  ...
  if (dst instanceof this.ScreenBuffer) {
    ...
    if (
      !this.ch &&
      (this.dstClipRect || new termkit.Rect(this.dst)).isInside(
        cx + this.x,
        this.cy + this.y,
      )
    ) {
    ...
    context: {
      ...
      writeAttr:
        this.ScreenBuffer === termkit.ScreenBuffer
          ? (dst, attr, offset) => {
              dst.writeInt32BE(attr, offset);
            }
          : (dst, attr, offset) => {
              attr.copy(dst, offset);
            },
    },
...
TextBuffer.prototype.blitterLineIterator = function (p) {
  ...
  if (cells) {
    ...
    for (
      ;
      srcX <= srcExistingXmax;
      srcX++, dstOffset += this.ScreenBuffer.prototype.ITEM_SIZE
    ) {
      if (srcX < 0) {
        continue;
      } // right-shifted
      ...
      if (p.context.forceChar) {
        ...
        attr =
          cell.width === 2
            ? cell.attr | this.ScreenBuffer.prototype.LEADING_FULLWIDTH
            : cell.width === 0
              ? cell.attr | this.ScreenBuffer.prototype.TRAILING_FULLWIDTH
              : cell.attr;
        char =
      ...
      if (srcX >= invXmin && srcX <= invXmax) {
        attr = this.ScreenBuffer.attrInverse(attr);
      }

      // Write the attributes
      p.context.writeAttr(p.context.dstBuffer, attr, dstOffset);
      // Write the char
      p.context.dstBuffer.write(
        char,
        dstOffset + this.ScreenBuffer.prototype.ATTR_SIZE,
        this.ScreenBuffer.prototype.CHAR_SIZE,
      );
    }
  }
...
TextBuffer.prototype.object2attr = function (
  attrObject,
  colorNameToIndex = this.palette?.colorNameToIndex,
  legacyColor = false,
) {
  return this.ScreenBuffer.object2attr(
    attrObject,
    colorNameToIndex,
    legacyColor,
  );
...
TextMachineApi.style = (context, style) => {
  if (!context || context.x === null) {
    return;
  } // This is a newline or end of buffer character, there is no style to apply here
  if (!style.code) {
    style.code = context.textBuffer.ScreenBuffer.object2attr(style);
  } // cache it now
...
TextMachineApi.blockStyle = function (startingContext, endingContext, style) {
  if (
    !startingContext ||
    !endingContext ||
    startingContext.x === null ||
    endingContext.x === null
  ) {
    return;
  }
  if (!style.code) {
    style.code = startingContext.textBuffer.ScreenBuffer.object2attr(style);
  } // cache it now
