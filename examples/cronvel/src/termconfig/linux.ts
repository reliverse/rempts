...
const fgCursorTable = [0, 3, 5, 1, 6, 2, 4, 7, 8, 11, 13, 9, 14, 10, 12, 15];

const bgCursorTable = [0, 4, 2, 6, 1, 5, 3, 7, 8, 12, 10, 14, 9, 13, 11, 15];
...
const esc = tree.extend(null, Object.create(xterm.esc), {
  ...
  setCursorColor: {
    ...
    handler: function (bg, fg) {
      ...
      // If the register is greater than 15, find the 0..15 register that is close to it
      if (fg > 15) {
        fg = this.root.registerForRgb(this.root.rgbForRegister(fg), 0, 15);
      }
      if (bg > 15) {
        bg = this.root.registerForRgb(this.root.rgbForRegister(bg), 0, 15);
      }
...
function gpmMouse(mode) {
  ...
  this.root.gpmHandler = gpm.createHandler({
    stdin: this.root.stdin,
    raw: false,
    mode: mode,
  });
