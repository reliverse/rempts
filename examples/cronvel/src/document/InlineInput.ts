...
function InlineInput(options) {
  ...
  // No scrolling
  options.scrollable =
    options.hasVScrollBar =
    options.hasHScrollBar =
    options.extraScrolling =
      false;
  ...
  this.onAutoCompleteMenuItemFocus =
    this.onAutoCompleteMenuItemFocus.bind(this);
  ...
  if (options.prompt) {
    ...
    if (size.height > 1) {
      ...
      options.firstLineRightShift =
        this.promptTextBox.textBuffer.buffer[
          this.promptTextBox.textBuffer.buffer.length - 1
        ].length;
    } else {
  ...
  this.contentArray = options.history
    ? [...options.history, this.content]
    : [this.content];
  ...
  this.useAutoCompleteHint = !!(
    this.autoComplete &&
    (options.useAutoCompleteHint || options.autoCompleteHint)
  );
  this.autoCompleteHintMinInput = options.autoCompleteHintMinInput || 1; // number of input chars before starting to hint
  this.useAutoCompleteMenu = !!(
    this.autoComplete &&
    (options.useAutoCompleteMenu || options.autoCompleteMenu)
  );
  ...
  if (this.elementType === "InlineInput" && !options.noDraw) {
    this.draw();
  }
}
...
InlineInput.prototype.autoResizeAndDraw = function (onlyDrawCursor = false) {
  var height = Math.max(
    ...
    (this.altTextBuffer && this.altTextBuffer.buffer.length) || 0,
  );
...
InlineInput.prototype.runAutoCompleteHint = async function (autoComplete) {
  if (
    this.textBuffer.cy === 0 &&
    this.textBuffer.cx < this.autoCompleteHintMinInput
  ) {
  ...
  if (Array.isArray(autoCompleted)) {
    ...
    autoCompleted =
      (autoCompleted.prefix ?? "") +
      autoCompleted[0] +
      (autoCompleted.postfix ?? "");
  }
...
InlineInput.prototype.runAutoComplete = async function (autoComplete) {
  ...
  [this.autoCompleteLeftPart, this.autoCompleteRightPart] =
    this.textBuffer.getCursorSplittedText();

  if (Array.isArray(autoComplete)) {
    autoCompleted = computeAutoCompleteArray(
      autoComplete,
      this.autoCompleteLeftPart,
      this.useAutoCompleteMenu,
    );
  } else if (typeof autoComplete === "function") {
    autoCompleted = await autoComplete(
      this.autoCompleteLeftPart,
      this.useAutoCompleteMenu,
    );
  } else {
...
InlineInput.prototype.runAutoCompleted = async function (autoCompleted) {
  if (autoCompleted.startsWith(this.autoCompleteLeftPart)) {
    this.textBuffer.insert(
      autoCompleted.slice(this.autoCompleteLeftPart.length),
    );
...
InlineInput.prototype.onAutoCompleteMenuSubmit = function (selectedText) {
  selectedText =
    this.autoCompleteMenuPrefix + selectedText + this.autoCompleteMenuPostfix;
...
InlineInput.prototype.onAutoCompleteMenuItemFocus = function (
  selectedText,
  focus,
) {
  if (!focus || this.autoCompleteRightPart) {
    return;
  }
  selectedText =
    this.autoCompleteMenuPrefix + selectedText + this.autoCompleteMenuPostfix;
...
userActions.cancel = function () {
  if (!this.cancelable || this.disabled || this.canceled) {
    return;
  }
  //this.canceled = true ;
...
userActions.historyNext = function () {
  if (this.contentIndex >= this.contentArray.length - 1) {
    return;
  }
  this.contentArray[this.contentIndex] = this.getContent();
  this.contentIndex++;
