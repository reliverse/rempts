...
function TextBox(options) {
  ...
  this.altTextAttr =
    options.altTextAttr ||
    Object.assign({}, this.textAttr, { color: "gray", italic: true });
  this.voidAttr = options.voidAttr ||
    options.emptyAttr ||
    options.attr || { bgColor: "default" };
  ...
  this.textAreaWidth = this.hasVScrollBar
    ? this.outputWidth - 1
    : this.outputWidth;
  this.textAreaHeight = this.hasHScrollBar
    ? this.outputHeight - 1
    : this.outputHeight;
  ...
  if (this.elementType === "TextBox" && !options.noDraw) {
    this.draw();
  }
}
...
TextBox.prototype.initChildren = function () {
  ...
  if (this.hasVScrollBar) {
    this.vScrollBarSlider = new Slider({
      ...
      valueToRate: (scrollY) =>
        -scrollY /
        Math.max(1, this.textBuffer.buffer.length - this.textAreaHeight),
      rateToValue: (rate) =>
        -rate *
        Math.max(1, this.textBuffer.buffer.length - this.textAreaHeight),
      noDraw: true,
    ...
    this.vScrollBarSlider.on("slideStep", (d) =>
      this.scroll(0, -d * Math.ceil(this.textAreaHeight / 2)),
    );
  ...
  if (this.hasHScrollBar) {
    ...
    this.hScrollBarSlider.on("slideStep", (d) =>
      this.scroll(-d * Math.ceil(this.textAreaWidth / 2), 0),
    );
...
TextBox.prototype.setSizeAndPosition = function (options) {
  this.outputX =
    options.outputX !== undefined
      ? options.outputX
      : options.x !== undefined
        ? options.x
        : this.outputX || 0;
  this.outputY =
    options.outputY !== undefined
      ? options.outputY
      : options.y !== undefined
        ? options.y
        : this.outputY || 0;
  ...
  if (this.altTextBuffer) {
    this.altTextBuffer.lineWrapWidth = this.lineWrap
      ? this.textAreaWidth
      : null;
  }
  ...
  if (this.lineWrap) {
    ...
    if (this.altTextBuffer) {
      this.altTextBuffer.wrapAllLines();
    }
  }
...
TextBox.prototype.scrollTo = function (x, y, noDraw = false) {
  ...
  if (x !== undefined && x !== null) {
    ...
    this.scrollX = Math.min(
      ...
      Math.max(
        Math.round(x),
        (this.extraScrolling ? 1 : this.textAreaWidth) -
          this.textBuffer.getContentSize().width +
          1,
      ),
    );
  ...
  if (y !== undefined && y !== null) {
    this.scrollY = Math.min(
      ...
      Math.max(
        Math.round(y),
        (this.extraScrolling ? 1 : this.textAreaHeight) -
          this.textBuffer.buffer.length,
      ),
    );
...
TextBox.prototype.scroll = function (dx, dy, dontDraw = false) {
  return this.scrollTo(
    dx ? this.scrollX + dx : null,
    dy ? this.scrollY + dy : null,
    dontDraw,
  );
};
...
TextBox.prototype.scrollToBottom = function (dontDraw = false) {
  ...
  return this.scrollTo(
    null,
    this.textAreaHeight - this.textBuffer.buffer.length,
    dontDraw,
  );
};

TextBox.prototype.autoScrollAndDraw = function (
  onlyDrawCursorExceptIfScrolled = false,
  noDraw = false,
) {
  var x,
    y,
    contextColumns = Math.min(
      Math.floor(this.textAreaWidth / 2),
      this.autoScrollContextColumns,
    ),
    contextLines = Math.min(
      Math.floor(this.textAreaHeight / 2),
      this.autoScrollContextLines,
    );

  // Do nothing if there is no scrolling yet (do not set x to 0 if it's unnecessary)
  if (
    this.textBuffer.cx < -this.scrollX + contextColumns &&
    this.scrollX !== 0
  ) {
    ...
    this.textBuffer.cx >
    this.textAreaWidth - this.scrollX - 1 - contextColumns
  ) {
  ...
  if (this.textBuffer.cy < -this.scrollY + contextLines) {
    ...
    this.textBuffer.cy >
    this.textAreaHeight - this.scrollY - 1 - contextLines
  ) {
...
TextBox.prototype.autoScrollAndSmartDraw = function () {
  return this.autoScrollAndDraw(true);
};

TextBox.prototype.setAttr = function (
  textAttr = this.textAttr,
  voidAttr = this.voidAttr,
  dontDraw = false,
  dontSetContent = false,
) {
  ...
  if (!dontSetContent) {
    this.setContent(this.content, this.contentHasMarkup, dontDraw);
  }
};
...
TextBox.prototype.prependContent = function (content, dontDraw) {
  return this.addContent(content, "prepend", dontDraw);
};
TextBox.prototype.appendContent = function (content, dontDraw) {
  return this.addContent(content, "append", dontDraw);
};
TextBox.prototype.appendLog = function (content, dontDraw) {
  return this.addContent(content, "appendLog", dontDraw);
};
...
TextBox.prototype.addContent = function (content, mode, dontDraw) {
  ...
  switch (mode) {
    ...
    case "appendLog":
      ...
      scroll =
        this.textBuffer.buffer.length <= this.textAreaHeight ||
        this.scrollY <= this.textAreaHeight - this.textBuffer.buffer.length;
      this.textBuffer.moveToEndOfBuffer();
...
TextBox.prototype.setTabWidth = function (tabWidth, internal = false) {
  ...
  if (this.altTextBuffer) {
    this.altTextBuffer.setTabWidth(this.tabWidth);
  }
...
TextBox.prototype.onDrag = function (data) {
  ...
  if (this.document) {
    this.document
      .setSystemClipboard(this.textBuffer.getSelectionText(), "primary")
      .catch(() => undefined);
  }
...
userActions.copyToSystemClipboard = function () {
  if (this.document) {
    this.document
      .setSystemClipboard(this.textBuffer.getSelectionText())
      .catch(() => undefined);
  }
};
