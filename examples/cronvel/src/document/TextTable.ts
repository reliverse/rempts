...
function TextTable(options) {
  ...
  if (typeof options.borderChars === "object") {
    ...
    typeof options.borderChars === "string" &&
...
TextTable.prototype.setCellContent = function (
  x,
  y,
  content,
  dontDraw = false,
  dontUpdateLayout = false,
) {
...
TextTable.prototype.setCellAttr = function (
  x,
  y,
  textAttr,
  voidAttr,
...
TextTable.prototype.setRowAttr = function (
  y,
  textAttr,
  voidAttr,
  dontDraw = false,
...
TextTable.prototype.setColumnAttr = function (
  x,
  textAttr,
  voidAttr,
  dontDraw = false,
...
TextTable.prototype.setTableAttr = function (
  textAttr,
  voidAttr,
  dontDraw = false,
) {
...
TextTable.prototype.preDrawSelf = function () {
  ...
  for (j = 0; j < this.rowHeights.length; j++) {
    ...
    for (i = 0; i < this.columnWidths.length; i++) {
      ...
      this.outputDst.put(
        { x, y, attr: this.borderAttr },
        this.borderChars.horizontal.repeat(this.columnWidths[i]),
      );
    ...
    this.outputDst.put(
      { x, y, attr: this.borderAttr },
      j ? this.borderChars.rightTee : this.borderChars.topRight,
    );
  ...
  for (i = 0; i < this.columnWidths.length; i++) {
    ...
    this.outputDst.put(
      { x, y, attr: this.borderAttr },
      i ? this.borderChars.bottomTee : this.borderChars.bottomLeft,
    );
    x++;
