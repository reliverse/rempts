...
Terminal.create = function (createOptions) {
  ...
  term.colorRegister = tree.extend(
    { deep: true },
    [],
    defaultColorRegister,
    termconfig.colorRegister,
  );
    ...
    for (j = 0; j < codeList.length; j++) {
      ...
      if (code) {
        ...
        if (keymapObject.starter.length > term.rKeymapStarterMaxSize) {
          for (
            i = term.rKeymapStarterMaxSize + 1;
            i <= keymapObject.starter.length;
            i++
          ) {
        ...
        if (
          term.rKeymapStarter[keymapObject.starter.length][keymapObject.starter]
        ) {
          term.rKeymapStarter[keymapObject.starter.length][
            keymapObject.starter
          ].push(key);
        } else {
          term.rKeymapStarter[keymapObject.starter.length][
  ...
  term.setResetString = function (str) {
    term.resetString = string.markupMethod.call(
      term.formatConfig.rawMarkupConfig,
      str,
    );
  };
      ...
      if (value === undefined) {
        if (key[0] === "#") {
          ...
          switch (termkit.markupCatchAllKeywords[key][0]) {
            case "color":
              return term.formatConfig.dataMarkup.fg(
                markupStack,
                "fg",
                termkit.markupCatchAllKeywords[key][1],
              );
            case "bgColor":
              return term.formatConfig.dataMarkup.bg(
                markupStack,
                "bg",
                termkit.markupCatchAllKeywords[key][1],
              );
...
function applyEscape(options, ...args) {
  ...
  if (options.wrap) {
    if (options.root.wrapOptions.x && options.root.wrapOptions.x > 1) {
      wrapOptions = {
        width:
          options.root.wrapOptions.width ||
...
notChainable.table = function (table, options = {}) {
  return termkit.Element.createInline(
    this,
    termkit.TextTable,
  ...
  while (index < length) {
    ...
    if (chunk[index] <= 0x1f || chunk[index] === 0x7f) {
      ...
      for (
        i = Math.min(
          length,
          Math.max(this.rKeymapMaxSize, this.rKeymapStarterMaxSize),
        );
        i > 0;
        i--
      ) {
        ...
        if (this.rKeymap[i] && this.rKeymap[i][keymapStartCode]) {
          ...
          if (keymap.handler) {
            handlerResult = keymap.handler.call(
              this,
              keymap.name,
              chunk.slice(index + i),
            );
...
notChainable.setPalette = function (palette) {
  ...
  if (typeof palette === "string") {
    try {
      ...
      throw new Error(
        "[terminal] .setPalette(): color scheme not found: " + palette,
      );
    }
  }
...
notChainable.getTerminfo = function (key) {
  ...
  if (this.lock.getTerminfo) {
    this.once("unlock_getTerminfo", () =>
      this.getTerminfo(key).propagate(promise),
    );
    return promise;
  }
...
notChainable.registerForRgbCache = function (
  cache,
  r,
  g,
  b,
    ...
    for (x = 0; x < xMax; x++) {
      if (this.support.trueColor) {
        ...
        if (y * 2 + 1 < pixels.shape[1]) {
          ...
          this.noFormat(
            this.optimized.color256[fgColor] +
...
notChainable.drawImage = function (filepath, options, callback) {
  return termkit.image.load.call(
    this,
    notChainable.drawNdarrayImage.bind(this),
    filepath,
    options,
    callback,
  );
