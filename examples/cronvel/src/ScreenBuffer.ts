...
ScreenBuffer.createFromString = function (options, data) {
  ...
  if (typeof data !== "string") {
    if (!data.toString) {
      throw new Error(
        "[terminal] ScreenBuffer.createFromDataString(): argument #1 should be a string or provide a .toString() method.",
      );
    }
    data = data.toString();
  }
...
ScreenBuffer.prototype.parseMarkup = string.markupMethod.bind(
  misc.markupOptions,
);
...
ScreenBuffer.prototype.put = function (options, str, ...args) {
  ...
  var processRaw = (part) => {
    ...
    for (let i = 0; i < iMax; i++) {
      ...
      if (!inBounds && lastValidOffset >= 0 && options.clipChar) {
        // Check if we are writing on a fullwidth char
        if (
          this.hasLeadingFullWidth(this.buffer, lastValidOffset) &&
          x < xmax
        ) {
          this.removeFullWidth(this.buffer, lastValidOffset + this.ITEM_SIZE);
        } else if (
          this.hasTrailingFullWidth(this.buffer, lastValidOffset) &&
          x
        ) {
          this.removeFullWidth(this.buffer, lastValidOffset - this.ITEM_SIZE);
        }
  ...
  if (!options.markup) {
    ...
    for (let part of parts) {
      attrObject = Object.assign(
        {},
        part.specialReset ? defaultAttrObject : baseAttrObject,
        part,
      );
...
ScreenBuffer.prototype.resize = function (fromRect) {
  ...
  var resizedBuffer = Buffer.allocUnsafe(
    toRect.width * toRect.height * this.ITEM_SIZE,
  );
...
ScreenBuffer.prototype.draw = function (options) {
  ...
  var tr = {
    ...
    offsetX:
      options.x !== undefined ? Math.floor(options.x) : Math.floor(this.x),
    offsetY:
      options.y !== undefined ? Math.floor(options.y) : Math.floor(this.y),
...
ScreenBuffer.prototype.blitterLineIterator = function (p) {
  if (p.dstStart >= this.ITEM_SIZE) {
    ...
    this.removeLeadingFullWidth(
      p.context.dstBuffer,
      p.dstStart - this.ITEM_SIZE,
    );
  }
    ...
    if ((attr & ATTR_MASK) !== (p.context.lastAttr & ATTR_MASK)) {
      p.context.sequence +=
        p.context.lastAttr === null || !p.context.deltaEscapeSequence
          ...
          : this.generateDeltaEscapeSequence(
              p.context.term,
              attr,
              p.context.lastAttr,
            );
...
ScreenBuffer.loadImage = termkit.image.load.bind(
  ScreenBuffer,
  ScreenBuffer.fromNdarrayImage,
);
...
ScreenBuffer.prototype.generateDeltaEscapeSequence = function (
  term,
  attr,
  lastAttr,
) {
...
*/
// Shared
ScreenBuffer.prototype.vScroll = function (
  lineOffset,
  attr,
  ymin,
  ymax,
...
ScreenBuffer.object2attr = function (
  object,
  colorNameToIndex,
  legacyColor = false,
) {
...
ScreenBuffer.prototype.object2attr = function (
  object,
  colorNameToIndex = this.palette && this.palette.colorNameToIndex,
  legacyColor = false,
) {
...
ScreenBuffer.prototype.attrAndObject = function (attr, object) {
  return ScreenBuffer.attrAndObject(
    attr,
    object,
    this.palette && this.palette.colorNameToIndex,
  );
};

// Used by TextBuffer for selection
ScreenBuffer.attrInverse = ScreenBuffer.prototype.attrInverse = (attr) =>
  attr ^ INVERSE;
...
const TRANSPARENCY =
  FG_TRANSPARENCY | BG_TRANSPARENCY | STYLE_TRANSPARENCY | CHAR_TRANSPARENCY;
...
ScreenBuffer.prototype.CLEAR_BUFFER = Buffer.allocUnsafe(
  ScreenBuffer.prototype.ITEM_SIZE,
);
ScreenBuffer.prototype.CLEAR_BUFFER.writeInt32BE(
  ScreenBuffer.prototype.CLEAR_ATTR,
  0,
);
ScreenBuffer.prototype.CLEAR_BUFFER.write(
  " \x00\x00\x00",
  ScreenBuffer.prototype.ATTR_SIZE,
); // space
...
ScreenBuffer.loadSyncV2 = function (filepath) {
  ...
  try {
    ...
    throw new Error(
      "No correct one-lined JSON header found: this is not a ScreenBuffer file",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Missing mandatory header data, this is a corrupted or obsolete ScreenBuffer file",
    );
  }
