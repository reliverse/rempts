...
(async () => {
  const response = await prompts({
    ...
    validate: (value) => (value < 0 ? `Cant be less than zero` : true),
  });
  const HasPropValue = true;
  const DoesntHavePropAsdf = false;
})();
(async () => {
  await prompts([
    {
    ...
    {
      type: (prev, values) => {
        const HasPromptName = true;
        const DoesntHavePromptTypes = false;
