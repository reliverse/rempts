...
describe("multiselect", function () {
  ...
  describe("options.initial", () => {
    ...
    it("should not render the initial value on the prompt line", (cb) => {
      ...
      prompt.once("run", async () => {
        let init = stripAnsi(
          [prompt.symbols.pointer, prompt.options.initial].join(" "),
        );
        await prompt.render();
        try {
  ...
  describe("rendering", () => {
    it("should render a choice with the correct styles", (cb) => {
      ...
      prompt.on("run", async () => {
        ...
        assert.equal(
          await prompt.renderChoice(prompt.choices[0], 0),
          `${pointer} ${key}`,
        );
        assert.equal(
          await prompt.renderChoice(prompt.choices[1], 1),
          `${pointer} bar`,
        );
        prompt.submit();
      });
