...
describe("prompt-autocomplete", () => {
  describe("keypress > number", () => {
    ...
    it("should not crash when pressing space when there are no suggestions and using multiple", function (done) {
      ...
      prompt = new Prompt({
        ...
        multiple: true,
        choices: ["@changesets/cli", "@changesets/assemble-release-plan"],
      });
  ...
  describe("options.suggest", () => {
    ...
    it("should dynamically add choices", function () {
      ...
      return prompt.run().then(async (answer) => {
        assert.deepEqual(
          prompt.state._choices.map((ch) => ch.name),
          [
            "almond",
            "apple",
            "banana",
    ...
    it("should support async options.suggest function", () => {
      prompt = new Prompt({
        ...
        suggest(typed, choices) {
          const search = async () =>
            choices.filter((choice) => choice.message.includes(typed));
