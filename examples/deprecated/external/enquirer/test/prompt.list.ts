...
describe("list", function () {
  ...
  describe("usage", () => {
    it("should get a list of keywords", () => {
      prompt = new Prompt({
        message: "Enter a list of comma separated keywords:",
      });
    ...
    it("should get a list of email address", () => {
      prompt = new Prompt({
        message:
          "Enter email address either separated by a comma (,) or semicolon (;):",
        separator: /[,;]/,
        result(list) {
      ...
      prompt.once("run", async () => {
        prompt.state.input =
          "brian.woodward@gmail.com;doowb@example.com,jon@example.com;";
        prompt.submit();
      });
