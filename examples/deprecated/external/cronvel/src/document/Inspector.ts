...
Inspector.prototype.initChildren = function (noInitLevel = false) {
  ...
  this.buttonsDef.forEach((def) => {
    def.contentWidth = Element.computeContentWidth(
      def.content,
  ...
  buttonSpacing = Math.floor(
    (this.outputWidth - buttonsTextWidth) / (this.buttonsDef.length + 1),
  );
...
Inspector.prototype.resetChildren = function () {
  if (
    !this.valueFieldTextList.length &&
...
Inspector.prototype.onPropertiesColumnMenuSubmit = function (
  buttonValue,
  action,
  menu,
  button,
...
Inspector.prototype.onBreadCrumbRowMenuSubmit = function (
  buttonValue,
  action,
  menu,
  button,
