...
function ColumnMenu(options) {
  ...
  if (!options.outputWidth && !options.width) {
    if (options.parent) {
      options.outputWidth = Math.min(
        options.parent.inputWidth,
  ...
  if (this.elementType === "ColumnMenu" && !options.noDraw) {
    this.draw();
  }
}
...
ColumnMenu.prototype.initChildren = function (noInitPage = false) {
  ...
  this.previousPageDef = Object.assign(
    { content: "▲", internalRole: "previousPage" },
    this.previousPageDef,
  );
  this.previousPageDef.contentHasMarkup =
    this.previousPageDef.contentHasMarkup || this.previousPageDef.markup;
  this.previousPageDef.width =
    this.buttonPaddingWidth +
    Element.computeContentWidth(
  ...
  this.nextPageDef = Object.assign(
    { content: "▼", internalRole: "nextPage" },
    this.nextPageDef,
  );
  this.nextPageDef.contentHasMarkup =
    this.nextPageDef.contentHasMarkup || this.nextPageDef.markup;
  this.nextPageDef.width =
    this.buttonPaddingWidth +
    Element.computeContentWidth(
      this.nextPageDef.content,
      this.nextPageDef.contentHasMarkup,
    );
  this.nextPageDef.buttonContent = this.nextPageDef.content;

  if (this.masterDef) {
    this.masterDef = Object.assign(
      { content: "column-menu", internalRole: "master" },
      this.masterDef,
    );
    this.masterDef.contentHasMarkup =
      this.masterDef.contentHasMarkup || this.masterDef.markup;

    this.masterDef.buttonContent = this.masterDef.content;

    if (this.masterDef.symbol) {
      this.buttonSymbolWidth =
        1 + Element.computeContentWidth(this.masterDef.symbol);
      this.masterDef.buttonContent += " " + this.masterDef.symbol;
    }

    this.masterDef.width =
      this.buttonPaddingWidth +
      Element.computeContentWidth(
  ...
  this.buttonsMaxWidth = Math.max(
    this.buttonsMaxWidth,
    this.previousPageDef.width,
    this.nextPageDef.width,
    this.masterDef ? this.masterDef.width : 0,
  );
  ...
  this.itemsDef.forEach((def, index) => {
    ...
    var contentWidth = Element.computeContentWidth(
        def.content,
      ...
      pageMaxInnerHeight = Math.max(
        1,
        !isLastItem ? this.maxHeight - 1 : this.maxHeight,
      ),
      overflow = this.buttonPaddingWidth + contentWidth - this.outputWidth;

    if (overflow > 0) {
      if (this.multiLineItems) {
        def.buttonContent = Element.wordWrapContent(
          def.content,
          this.outputWidth - this.buttonPaddingWidth,
          def.contentHasMarkup,
        );
        ...
        def.buttonContent =
          Element.truncateContent(
            def.content,
            contentWidth - overflow - ellipsisWidth,
            def.contentHasMarkup,
          ) + this.contentEllipsis;
        contentWidth = Element.computeContentWidth(
          def.buttonContent,
          def.contentHasMarkup,
        );
      }
    ...
    if (!this.pageItemsDef[page]) {
      this.pageItemsDef[page] = [];
    }
  ...
  if (this.separatorDef) {
    this.separatorDef = Object.assign(
      { content: "-", disabled: true, internalRole: "separator" },
      this.separatorDef,
    );
    this.separatorDef.width = Element.computeContentWidth(
      this.separatorDef.content,
      this.separatorDef.contentHasMarkup,
    );

    if (
      this.separatorDef.contentRepeat &&
      this.separatorDef.width < this.buttonsMaxWidth - this.buttonPaddingWidth
    ) {
      this.separatorDef.content = this.separatorDef.content.repeat(
        Math.floor(
          (this.buttonsMaxWidth - this.buttonPaddingWidth) /
            this.separatorDef.width,
        ),
      );
      this.separatorDef.width = Element.computeContentWidth(
        this.separatorDef.content,
  ...
  if (this.masterDef && this.masterDef.width < this.buttonsMaxWidth) {
    this.masterDef.buttonContent =
      this.masterDef.content +
      " " +
      " ".repeat(this.buttonsMaxWidth - this.masterDef.width) +
      this.masterDef.symbol;
    this.masterDef.width = this.buttonsMaxWidth;
  }

  // Force at least an empty page
  if (!this.pageItemsDef.length) {
    this.pageItemsDef.push([]);
  }

  this.pageItemsDef.forEach((pageDef, index) => {
    if (index) {
      pageDef.unshift(this.previousPageDef);
    }
    if (index < this.pageItemsDef.length - 1) {
      pageDef.push(this.nextPageDef);
    }
    if (this.separatorDef) {
      pageDef.unshift(this.separatorDef);
    }
    if (this.masterDef) {
      pageDef.unshift(this.masterDef);
    }
  });
  ...
  if (this.elementType === "ColumnMenu" && !noInitPage) {
    this.initPage();
  }
};
...
ColumnMenu.prototype.initPage = function (page = this.page) {
  ...
  this.pageItemsDef[page].forEach((def, index) => {
    ...
    ButtonConstructor = def.internalRole
      ? Button
      : def.type === "button"
        ? Button
        : def.type === "toggle"
    ...
    isToggle =
      ButtonConstructor === ToggleButton ||
      ButtonConstructor.prototype instanceof ToggleButton;

    key = def.key; // For ToggleButton
    value =
      this.childUseParentKeyValue &&
      key &&
      this.value &&
      typeof this.value === "object"
        ? this.value[key]
        : def.value === undefined && !isToggle && def.key
          ? def.key
          : def.value;
    ...
    this.buttons[index] = new ButtonConstructor({
      ...
      turnedOffFocusAttr:
        def.turnedOffFocusAttr || this.buttonTurnedOffFocusAttr,
      ...
      keyBindings: isToggle
        ? this.toggleButtonKeyBindings
        : this.buttonKeyBindings,
      actionKeyBindings: isToggle
        ? this.toggleButtonActionKeyBindings
        : this.buttonActionKeyBindings,
    ...
    if (def.hotkey) {
      if (Array.isArray(def.hotkey)) {
        def.hotkey.forEach((hotkey) =>
          this.hotkeyToButtonIndex.set(hotkey, index),
        );
      } else {
  ...
  if (buttonOffsetY < this.outputHeight) {
    this.needOuterDraw = true;
  }
