...
function DropDownMenu(options) {
  ...
  this.columnButtonBlurAttr = options.buttonBlurAttr || {
    bgColor: "gray",
    color: "white",
    bold: true,
  };
  this.columnButtonFocusAttr = options.buttonFocusAttr || {
    bgColor: "blue",
    color: "white",
    bold: true,
  };
  this.columnButtonTurnedOnBlurAttr = options.buttonTurnedOnBlurAttr || {
    bgColor: "gray",
    color: "white",
    bold: true,
  };
  this.columnButtonTurnedOnFocusAttr = options.buttonTurnedOnFocusAttr || {
    bgColor: "blue",
    color: "white",
    bold: true,
  };
  this.columnButtonTurnedOffBlurAttr = options.buttonTurnedOffBlurAttr || {
    bgColor: "gray",
    color: "white",
    dim: true,
  };
  this.columnButtonTurnedOffFocusAttr = options.buttonTurnedOffFocusAttr || {
    bgColor: "blue",
    color: "white",
    dim: true,
  };
  ...
  if (this.elementType === "DropDownMenu" && !options.noDraw) {
    this.draw();
  }
}
...
DropDownMenu.prototype.dropDown = function (
  index,
  x,
  y,
  ...
  if (this.columnMenu) {
    ...
    if (this.columnMenu.index === index) {
      return;
    }
...
DropDownMenu.prototype.setValue = function (value, noDraw) {
  ...
  if (this.columnMenu) {
    this.columnMenu.setValue(value, noDraw);
  }
};
...
DropDownMenu.prototype.setKeyValue = function (key, value, noDraw) {
  ...
  if (this.columnMenu) {
    this.columnMenu.setKeyValue(key, value, noDraw);
  }
};

DropDownMenu.prototype.setDropDownItem = function (
  topItemValue,
  dropDownItemValue,
  itemOptions,
) {
  ...
  var dropDownItem =
    topItem.items && topItem.items.find((e) => e.value === dropDownItemValue);
  if (!dropDownItem) {
    return false;
  }
  this.clearColumnMenu();
...
DropDownMenu.prototype.onColumnMenuSubmit = function (
  buttonValue,
  action,
  columnMenu,
  button,
  ...
  if (button instanceof ToggleButton) {
    ...
    columnMenu.once(
      "blinked",
      (buttonValue_, reserved, columnMenu_, button_) => {
        if (this.clearColumnMenuOnSubmit) {
          this.clearColumnMenu(true);
        }
...
userActions.dropDown = function () {
  if (this.columnMenu) {
    ...
    this.dropDown(
      this.lastFocusButton.childId,
      this.lastFocusButton.outputX,
      this.lastFocusButton.outputY + 1,
    );
  }
