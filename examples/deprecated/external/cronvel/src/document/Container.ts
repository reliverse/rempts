...
function Container(options) {
  ...
  this.object2attr = (object) =>
    ScreenBuffer.object2attr(
      object,
  ...
  if (this.elementType === "Container" && !options.noDraw) {
    this.draw();
  }
}
...
Container.prototype.initChildren = function () {
  ...
  if (this.hasVScrollBar) {
    this.vScrollBarSlider = new termkit.Slider({
      ...
      isVertical: true,
      valueToRate: (scrollY) =>
        -scrollY / Math.max(1, this.inputHeight - this.viewportHeight),
      rateToValue: (rate) =>
        -rate * Math.max(1, this.inputHeight - this.viewportHeight),
      noDraw: true,
    });

    this.vScrollBarSlider.on("slideStep", (d) =>
      this.scroll(0, -d * Math.ceil(this.viewportHeight / 2)),
    );
  ...
  if (this.hasHScrollBar) {
    this.hScrollBarSlider = new termkit.Slider({
      ...
      valueToRate: (scrollY) =>
        -scrollY / Math.max(1, this.inputWidth - this.viewportWidth),
      rateToValue: (rate) =>
        -rate * Math.max(1, this.inputWidth - this.viewportWidth),
      noDraw: true,
    });

    this.hScrollBarSlider.on("slideStep", (d) =>
      this.scroll(-d * Math.ceil(this.viewportWidth / 2), 0),
    );
...
Container.prototype.scrollTo = function (x, y, noDraw = false) {
  ...
  if (x !== undefined && x !== null) {
    ...
    this.scrollX = Math.min(
      0,
      Math.max(Math.round(x), this.viewportWidth - this.inputWidth + 1),
    );
  ...
  if (y !== undefined && y !== null) {
    this.scrollY = Math.min(
      0,
      Math.max(Math.round(y), this.viewportHeight - this.inputHeight),
    );
...
Container.prototype.onDrag = function (data) {
  if (!this.movable || (!data.dx && !data.dy)) {
    return;
  }
