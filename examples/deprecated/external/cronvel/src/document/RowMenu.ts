...
function RowMenu(options) {
  ...
  this.separatorHasMarkup = !!(
    options.separatorHasMarkup || options.buttonSeparatorHasMarkup
  );
  this.separatorAttr = Object.assign(
    {},
    this.backgroundAttr,
    options.separatorAttr || options.buttonSeparatorAttr,
  );
  ...
  if (this.elementType === "RowMenu" && !options.noDraw) {
    this.draw();
  }
}
...
RowMenu.prototype.initChildren = function (noInitPage = false) {
  ...
  this.separatorWidth = Element.computeContentWidth(
    this.separator,
    this.separatorHasMarkup,
  );
  ...
  this.pageItemsDef.forEach((pageDef, index) => {
    ...
    pageDef.buttonsAndSeparatorsWidth =
      pageDef.buttonsWidth + (pageDef.length - 1) * this.separatorWidth;
    pageDef.justifyWidth = Math.max(
      0,
      this.justify
        ? (this.outputWidth -
            this.leftMargin -
  ...
  if (this.elementType === "RowMenu" && !noInitPage) {
    this.initPage();
  }
};
...
RowMenu.prototype.initPage = function (page = this.page) {
  ...
  pageDef.forEach((def, index) => {
    ...
    buttonOffsetX +=
      this.buttons[index].outputWidth + this.separatorWidth + justifyWidth;
  });
...
RowMenu.prototype.preDrawSelf = function () {
  ...
  this.outputDst.put(
    {
      x: this.outputX + this.leftMargin,
      y: this.outputY,
      attr: this.backgroundAttr,
    },
    " ".repeat(this.outputWidth - this.leftMargin),
  );
      ...
      this.outputDst.put(
        {
          x:
            Math.round(
              button.outputX + button.outputWidth + nextButton.outputX,
            ) / 2,
          y: this.outputY,
