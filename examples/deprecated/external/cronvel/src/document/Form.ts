...
function Form(options) {
  ...
  if (this.elementType === "Form" && !options.noDraw) {
    this.draw();
  }
}
...
Form.prototype.initChildren = function () {
  ...
  this.inputsDef.forEach((def) => {
    ...
    if (def.labelWidth > labelMaxWidth) {
      labelMaxWidth = def.labelWidth;
    }
  });
    ...
    switch (def.type) {
      case "select":
        ...
        this.labeledInputs[index] = new LabeledInput({
          ...
          buttonSubmittedAttr:
            def.buttonSubmittedAttr || this.buttonSubmittedAttr,
...
Form.prototype.onFocus = function (focus, type) {
  ...
  if (focus) {
    ...
    process.nextTick(() => {
      if (this.focusChild) {
        this.document.giveFocusTo(this.focusChild, "delegate");
      } else {
