...
function Palette(options = {}) {
  ...
  this.adaptivePaletteDef = this.system
    ? null
    : options.adaptivePaletteDef || defaultAdaptivePaletteDef;
  this.extraPaletteDef = this.system
    ? null
    : options.extraPaletteDef || defaultExtraPaletteDef;
  this.escape = [];
  this.bgEscape = [];
  this.chromaColors = [];
  this.colorIndex = {};
...
Palette.prototype.generateAdaptive = function () {
  ...
  baseChromaColors = this.adaptivePaletteDef.map((color) =>
    termkit.chroma(color.code),
  );
    ...
    for (j = 2; j >= -3; j--) {
      ...
      for (i = 0; i < 12; i++) {
        ...
        this.addColor(
          register,
          chromaColor,
          this.adaptivePaletteDef[i].names,
          "@",
          suffix,
        );
        register++;
...
Palette.prototype.generateExtra = function () {
  ...
  for (i = 0; i < 13 && i < this.extraPaletteDef.length; i++) {
    this.addColor(
      register,
      termkit.chroma(this.extraPaletteDef[i].code),
      this.extraPaletteDef[i].names,
      "*",
    );
    register++;
  }
...
Palette.prototype.addColor = function (
  register,
  chromaColor,
  names,
  prefix = "",
...
Palette.prototype.clStep = function (
  chromaColor,
  cAdjust,
  lAdjust,
  fixRgb = true,
