...
Rect.prototype.set = function (data) {
  if (data.xmin !== undefined) {
    this.xmin = Math.floor(data.xmin);
  }
  if (data.xmax !== undefined) {
    this.xmax = Math.floor(data.xmax);
  }
  if (data.ymin !== undefined) {
    this.ymin = Math.floor(data.ymin);
  }
  if (data.ymax !== undefined) {
    this.ymax = Math.floor(data.ymax);
  }
...
Rect.regionIterator = function (p, iterator) {
  ...
  if (p.dstClipRect) {
    p.dstClipRect.clip(p.dstRect);
  } else {
    p.dstClipRect = new Rect(p.dstRect);
  }

  if (p.srcClipRect) {
    p.srcClipRect.clip(p.srcRect);
  } else {
    p.srcClipRect = new Rect(p.srcRect);
  }
  ...
  if (p.dstRect.isNull || p.srcClipRect.isNull || p.dstClipRect.isNull) {
    return;
  }
    ...
    case "line":
      for (dstJ = 0; dstJ < p.dstClipRect.height; dstJ++) {
        ...
        while (dstI < p.dstClipRect.width) {
          ...
          streak = Math.min(
            p.srcClipRect.width - srcI,
            p.dstClipRect.width - dstI,
          );
