...
function Vte(options = {}) {
  this.width =
    Math.floor(options.width) || (options.dst ? options.dst.width : 80);
  this.height =
    Math.floor(options.height) || (options.dst ? options.dst.height : 25);
  this.palette = options.palette || (this.dst && this.dst.palette);

  this.screenBuffer = new ScreenBuffer(
    Object.assign({}, options, this, { wrap: true }),
  );
...
Vte.prototype.run = function (command, args) {
  ...
  try {
    ...
    logRed(
      "'child_pty' optional dependency not found, using regular child_process.spawn()",
    );
...
Vte.prototype.putChar = function (char) {
  ...
  log(
    "putChar:",
    charCode <= 0x1f || charCode === 0x7f ? "(ctrl)" : char,
    charCode >= 0x10
      ? "\\x" + charCode.toString(16)
      : "\\x0" + charCode.toString(16),
    "at:",
    this.cx,
    this.cy,
  );
...
Vte.prototype.moveCursorTo = function (x, y, internal = false) {
  ...
  if (this.cx < 0) {
    this.cx = 0;
  } else if (this.cx >= this.width - 1) {
    this.cx = this.width - 1;
  }

  if (this.cy < 0) {
    this.cy = 0;
  } else if (this.cy >= this.height - 1) {
...
Vte.prototype.nextTab = function () {
  this.moveCursorTo(
    Math.ceil((this.cx + 1) / this.tabWidth) * this.tabWidth,
    undefined,
    true,
  );
};
...
Vte.prototype.vScroll = function (lineOffset, noDraw) {
  ...
  if (
    this.scrollingRegion &&
    this.cy >= this.scrollingRegion.ymin &&
    this.cy <= this.scrollingRegion.ymax
  ) {
...
Vte.prototype.newLine = function (noDraw) {
  return this.lineFeed(true, noDraw);
};
...
Vte.prototype.delete = function (count = 1) {
  if (count > this.width - this.cx) {
    count = this.width - this.cx;
  }
  if (count <= 0) {
    return;
  }
...
Vte.prototype.deleteLine = function (count = 1) {
  if (count > this.height - this.cy) {
    count = this.height - this.cy;
  }
  if (count <= 0) {
    return;
  }
...
Vte.prototype.resetAttr = function () {
  this.attr = this.screenBuffer.DEFAULT_ATTR;
};
Vte.prototype.setAttr = function (attrObject) {
  this.attr = this.screenBuffer.object2attr(attrObject);
};
Vte.prototype.addAttr = function (attrObject) {
  this.attr = this.screenBuffer.attrAndObject(this.attr, attrObject);
};

Vte.prototype.setVScrollingRegion = function (
  ymin = null,
  ymax = null,
  internal = false,
) {
  ...
  if (ymin === null || ymax === null) {
    ...
    this.scrollingRegion = new Rect(
      0,
      Math.max(0, ymin),
      this.width - 1,
      Math.min(this.height - 1, ymax),
    );
  } else {
    this.scrollingRegion = new Rect(
      0,
      Math.max(0, ymin - 1),
      this.width - 1,
      Math.min(this.height - 1, ymax - 1),
    );
  }
  log(
    "########################### setVScrollingRegion region:",
    this.scrollingRegion,
  );
};
...
Vte.prototype.emitCursorLocation = function (decVariant) {
  ...
  this.emit(
    "input",
    string.format(
      toInputSequence.reports[
        decVariant ? "cursorLocationDecVariant" : "cursorLocation"
      ],
      this.cx,
      this.cy,
    ),
...
Vte.prototype.emitScreenSize = function (decVariant) {
  this.emit(
    "input",
    string.format(toInputSequence.reports.screenSize, this.width, this.height),
  );
};
...
Vte.prototype.emitRegisterColor = function (register) {
  ...
  this.emit(
    "input",
    string.format(
      toInputSequence.reports.registerColor,
      register,
      rgb.r,
      rgb.g,
      rgb.b,
...
Vte.prototype.emitMouseSGR = function (type, data) {
  ...
  this.emit(
    "input",
    "\x1b[<" + code + ";" + data.x + ";" + data.y + (released ? "m" : "M"),
  );
};
...
Vte.prototype.onEventInputMouse = function (type, data) {
  ...
  switch (type) {
    ...
    case "MOUSE_MOTION":
      if (
        this.mouseEvent === "motion" ||
        (this.mouseEvent === "drag" && this.mouseIsDragging)
      ) {
        this.emitMouseSGR(type, data);
      }
      break;
