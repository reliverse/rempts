 ...
 */
export const INPUT_EXAMPLES: PromptType[] = [
  "input",
  "select",
  "multiselect",
  "date",
 ...
 * @param promptType The type of example to show.
...
export async function handleExample(
  ...
  try {
    const handler = exampleHandlers[promptType];
    if (handler) {
      await handler(userInput, selectedExamples);
    } else {
      console.warn(`Unhandled example type: ${promptType}`);
    }
  } catch (error: unknown) {
    console.error(`Error handling example ${promptType}:`, error);
    // Fallback to default value if available
    const key = promptType as keyof typeof DEFAULT_USER_INPUT;
