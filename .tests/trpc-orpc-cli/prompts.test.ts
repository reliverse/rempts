      ...
      return {
        setup: async (ctx) => {
          ...
          log({
            ...
            inputs: {
              arguments: ctx.inputs.arguments.map((a) => ({
                name: a.name,
                value: a.value,
                specified: a.specified,
              })),
              options: ctx.inputs.options.map((o) => ({
                name: o.name,
                value: o.value,
        ...
        input: async (params, ctx) => {
          const commanderOptions = (command as Command).options.filter(
            (o) => o.name() === ctx.option?.name(),
          );
  ...
  expect(log.mock.calls[0][0]).toMatchObject({
    inputs: {
      options: expect.arrayContaining([
        { name: "package-manager", specified: true, value: "yarn" },
      ]),
    },
  });
