...
export const testSuite: CodegenPreset = ({
  ...
  const parseTestFile = (content: string) => {
    ...
    const firstNonImportLine = lines.findIndex(
      (line) => line && !line.startsWith("import") && !line.startsWith("//"),
    );
  ...
  const parseTest = (test: { code: string }) => {
    ...
    for (const [chunkIndex, chunk] of chunks.slice(1).entries()) {
      ...
      code += placeholder;
      placeholders[chunkIndex] = {
        original: chunk.slice(0, indexOfClosedParen),
        placeholder,
      };
  ...
  let expected = zod3.tests
    .map((test) => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        if (i in parsed.placeholders && parsed.placeholders[i]) {
