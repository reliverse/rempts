...
test("tuple input with flags", async () => {
  ...
  `);
  await expect(
    run(router, ["foo", "hello", "not a number!", "--foo", "bar"]),
...
test("nullable array inputs aren't supported", async () => {
  ...
  await expect(
    run(router, ["test1", "--help"], { expectJsonInput: true }),
  ...
  await expect(
    run(router, ["test2", "--help"], { expectJsonInput: true }),
  ).resolves.toMatchInlineSnapshot(`
...
test("defaults and negations", async () => {
  const router = t.router({
    ...
    booleanOrString: t.procedure
      .input(type({ foo: type("boolean | string") }))
      .query(({ input }) => inspect(input)),
  ...
  expect(
    await run(router, ["array-of-boolean-or-number", "--foo", "true", "1"]),
  ).toMatchInlineSnapshot(`"{ foo: [ true, 1 ] }"`);
});
// codegen:end
