 ...
 * ```
 */
export function breakpoints(breakpointsMap: Record<string, Options>) {
  const bp = Object.keys(breakpointsMap)
    .map((key) => {
  ...
  return (stdoutColumns: number) =>
    bp.find(
      ({ operator, breakpoint }) =>
        (operator === "=" && stdoutColumns === breakpoint) ||
        (operator === ">" && stdoutColumns > breakpoint) ||
        (operator === "<" && stdoutColumns < breakpoint) ||
        (operator === ">=" && stdoutColumns >= breakpoint) ||
        (operator === "<=" && stdoutColumns <= breakpoint),
