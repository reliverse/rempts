// bun test src\components\columns\terminal-columns.test.ts

import { describe, test, expect, beforeAll } from "bun:test";
import { blue, bold, underline } from "colorette";

import { terminalColumns, breakpoints } from "~/columns/index.js";
...
describe("edge cases", () => {
  describe("error handling", () => {
    test("missing columns", () => {
      expect(() => terminalColumns([[""]], [100, 200])).toThrow(
        "2 columns defined, but only 1 columns found",
      );
    });

    test("invalid column", () => {
      expect(() => terminalColumns([[""]], ["100"])).toThrow(
        'Invalid column width: "100"',
      );
    });
  });
  ...
  test("inconsistent rows", () => {
    const table = terminalColumns([["A"], ["B", "B"], ["C", "C", "C"]]);
  ...
  test("infinite width", () => {
    const table = terminalColumns(
      [["A".repeat(100)], ["B", "B".repeat(100)], ["C", "C", "C".repeat(100)]],
      {
...
describe("process", () => {
  ...
  test("postprocess ignores vertical padding", () => {
    const table = terminalColumns(
      [[loremIpsumNewLines, loremIpsumNewLines]],
      [
        {
...
describe("auto", () => {
  test("event split", () => {
    const table = terminalColumns(
      [[loremIpsumShort, loremIpsumShort, loremIpsumShort]],
      ["auto", "auto", "auto"],
    );
