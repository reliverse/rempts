...
function resolveColumnWidths<T extends ColumnWidth>(
  ...
  stdoutWidth: number,
): asserts columnMetas is InternalColumnMeta<
  // eslint-disable-next-line @typescript-eslint/no-redundant-type-constituents
  Exclude<T, "content-width" | "auto" | string>
>[] {
  for (const column of columnMetas) {
    ...
    if (typeof width === "string" && isPercentPattern.test(width)) {
      ...
      column.width = (Math.floor(stdoutWidth * columnWidthPercent) -
    ...
    column.paddingLeftString = column.paddingLeft
      ? " ".repeat(column.paddingLeft)
      : "";
    column.paddingRightString = column.paddingRight
      ? " ".repeat(column.paddingRight)
      : "";
...
const makeRow = () =>
  Object.assign([] as InternalColumnMeta<number>[], { columns: 0 });
...
function balanceAuto(
  ...
  for (const columns of rows) {
    const usedWidth = columns.reduce(
      (sum, column) => sum + column.width + column.horizontalPadding,
      0,
    );
    ...
    const autoColumns = columns.filter(
      (column) => "autoOverflow" in column,
    ) as (InternalColumnMeta<number> & { autoOverflow: number })[];
    const widthOverflows = autoColumns.filter(
      (column) => column.autoOverflow > 0,
    );

    const widthOverflowTotal = widthOverflows.reduce(
      (sum, column) => sum + column.autoOverflow,
      0,
    );
    const overflowRestorationBudget = Math.min(
      widthOverflowTotal,
      remainingWidth,
    );
