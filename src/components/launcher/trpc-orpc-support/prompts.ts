...
interface UpstreamArgumentInfo {
...
export const createShadowCommand = (
  ...
  command.options.forEach((original) => {
    ...
    const upstreamOptionInfo: UpstreamOptionInfo = {
      typeName: "UpstreamOptionInfo",
      id,
      specified: false,
    };
    shadowOption.default(JSON.stringify(upstreamOptionInfo));
    shadowOption.argParser((value) =>
      JSON.stringify({ ...upstreamOptionInfo, specified: true, value }),
    );
  ...
  command.registeredArguments.forEach((original) => {
    ...
    const upstreamArgumentInfo: UpstreamArgumentInfo = {
      typeName: "UpstreamArgumentInfo",
      id,
      specified: false,
    };
    shadowArgument.default(JSON.stringify(upstreamArgumentInfo));
    shadowArgument.argParser((value) =>
      JSON.stringify({ ...upstreamArgumentInfo, specified: true, value }),
    );
...
const promptsPrompter = (prompts: PromptsLike): Prompter => {
  ...
  return {
    ...
    select: async (params) => {
      const choicesObjects = params.choices.map((c) =>
      ...
      return p({
        ...
        initial: params.default
          ? choicesObjects.findIndex((c) => c.value === params.default)
          : undefined,
      }).then(x<string>());
    },
    checkbox: async (params) => {
