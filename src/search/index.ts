...
type SearchConfig<Value, ChoicesObject = readonly (string | Separator)[]> = {
  message: string;
  source: (
    term: string | undefined,
    opt: { signal: AbortSignal },
  ) => ChoicesObject extends readonly (string | Separator)[]
    ? ChoicesObject | Promise<ChoicesObject>
    :
        | readonly (Choice<Value> | Separator)[]
        | Promise<readonly (Choice<Value> | Separator)[]>;
...
export const search = createPrompt(
  <Value>(config: SearchConfig<Value>, done: (value: Value) => void) => {
    ...
    const [searchResults, setSearchResults] = useState<readonly Item<Value>[]>(
      [],
    );
    ...
    const selectedChoice = searchResults[
      active
    ] as NormalizedChoice<Value> | void;
