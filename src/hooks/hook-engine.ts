...
function createStore(rl: ReadReli) {
...
function getStore() {
  ...
  if (!store) {
    throw new HookError(
      "Hook functions can only be called from within a prompt",
    );
  }
  return store;
}

export function readline(): ReadReli {
  return getStore().rl;
}
...
export const effectScheduler = {
  queue(cb: (readline: ReadReli) => void | (() => void)) {
    const store = getStore();
    const { index } = store;
