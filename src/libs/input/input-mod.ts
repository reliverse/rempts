...
export async function inputPrompt(options: InputPromptOptions): Promise<string> {
  ...
  async function handleHardcodedInput(): Promise<string> {
    ...
    await renderPromptUI({
      title: finalTitle,
      ...(hint !== undefined && { hint }),
      ...(hintPlaceholderColor !== undefined && { hintPlaceholderColor }),
      content: content ?? "",
      ...(contentColor !== undefined && { contentColor }),
      ...(contentTypography !== undefined && { contentTypography }),
      ...
      ...(customSymbol !== undefined && { customSymbol }),
      ...(symbolColor !== undefined && { symbolColor }),
      ...(mask !== undefined && { mask }),
      shouldStream,
      streamDelay,
    });
