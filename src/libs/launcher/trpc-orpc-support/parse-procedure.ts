...
function acceptedPrimitiveTypes(
  ...
  let constVals: string[] | undefined = [
...
function parseArrayInput(
  array: JSONSchema7 & { items: { type: unknown } },
): Result<ParsedProcedure> {
  ...
  return {
    ...
    value: {
      ...
      optionsJsonSchema: {},
      getPojoInput: (argv) =>
        (argv.positionalValues.at(-1) as string[]).map((s) =>
          convertPositional(array.items as JSONSchema7, s),
        ),
    },
  };
}
...
const convertPositional = (schema: JSONSchema7Definition, value: string) => {
  let preprocessed: string | number | boolean | undefined;
...
const parameterName = (s: JSONSchema7Definition, position: number): string => {
  ...
  let name =
    schemaDefPropValue(s, "title") ||
    schemaDefPropValue(s, "description") ||
    `parameter_${position}`;
  ...
  return {
    ...
    valibot: (input: unknown) => {
      ...
      const parent = valibotToJsonSchema(
        v.object({ child: input as import("valibot").StringSchema<undefined> }),
        {
          errorMode: "ignore",
        },
      );
    ...
    effect: (input: unknown) => {
      ...
      if (!effect.Schema.isSchema(input)) {
        const message =
          "input was not an effect schema - please use effect version 3.14.2 or higher. See https://github.com/mmkal/trpc-cli/pull/63";
