 ...
 */
export function createRpcCli<R extends AnyRouter>({
  router,
  ...params
  ...
  function buildProgram(runParams?: TrpcCliRunParams) {
    ...
    const commandTree: Record<string, TrpcCommand> = {
      "": program, // Root level
    };
        ...
        const getValueParser = (types: ReturnType<typeof getSchemaTypes>) => {
          ...
          if (types.length === 2 && types[0] === "boolean" && types[1] === "number") {
            return {
              type: "boolean|number",
              parser: (value: string) =>
                booleanParser(value, { fallback: null }) ?? numberParser(value),
            } as const;
          }
          if (types.length === 1 && types[0] === "boolean") {
            return {
              type: "boolean",
              parser: (value: string) => booleanParser(value),
            } as const;
          }
          if (types.length === 1 && types[0] === "number") {
            return {
              type: "number",
              parser: (value: string) => numberParser(value),
            } as const;
          }
        ...
        if (deprecatedCreateCaller) {
          ...
          caller = {
            [procedurePath]: (_input: unknown) =>
              call(procedure as never, _input, { context: params.context }),
          };
        } else {
          const createCallerFactor = resolvedTrpcServer.initTRPC.create()
    ...
    procedureEntries.forEach(([procedurePath, commandConfig]) => {
      ...
      for (let i = 0; i < segments.length - 1; i++) {
        ...
        if (!commandTree[currentPath]) {
          ...
          if (!parentCommand) continue; // Skip if parent doesn't exist
  ...
  const run: TrpcCli["run"] = async (
    runParams?: TrpcCliRunParams,
    program = buildProgram(runParams),
  ) => {
    if (!looksLikeInstanceof<TrpcCommand>(program, "TrpcCommand"))
    ...
    if (runParams?.prompts) {
      program = promptify(program, runParams.prompts) as TrpcCommand;
    }
    ...
    throw new FailedToExitError("Program exit after success", {
      exitCode: 0,
      cause: (program as TrpcCommand).__ran.at(-1)?.__result,
    });
  };
  ...
  return {
    run,
    buildProgram,
    toJSON: (program = buildProgram()) => commandToJSON(program as TrpcCommand),
  };
}
