...
import type { EditorExitResult } from "~/libs/core/core-types.js";
...
let state: EditorState = {
  ...
  theme: {
    ...
    highlight: (str) => re.invert(str), // For search results, etc.
...
function cutLine() {
  if (state.lines.length > 0) {
    ...
    // If we deleted the last line, add an empty one back
    if (state.lines.length === 0) {
      state.lines.push("");
    }
...
async function saveAsFile(): Promise<boolean> {
  ...
  if (newFilename) {
    ...
    return saveFile(); // Now call regular save
  }
...
if (isDirectRun) {
  ...
  startEditor({ filename: fileArg })
    .then((_result: EditorExitResult) => {
      ...
      // console.log(`Editor closed. Saved: ${_saved}. File: ${_filename || '[None]'}`);
    })
    .catch((error) => {
