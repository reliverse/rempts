...
export async function textPrompt(options, currentState = {
    ...
    const { title, stateCompletedTitle, hint, validate, defaultValue, schema, titleColor, titleTypography, content, contentColor, contentTypography, titleVariant, contentVariant, action, } = options;
    currentState.state = options.state ?? "initial";
    currentState.symbol = symbol("S_MIDDLE", currentState.state);
    const rl = readline.createInterface({ input, output });
    const coloredTitle = colorize(title, titleColor, titleTypography);
    const completedTitle = colorize(stateCompletedTitle ?? title, titleColor, titleTypography);
    ...
    function displayPrompt(isCompleted = false) {
        ...
        process.stdout.write(`${cursor.move(-999, 0)}${erase.line}${currentState.symbol} ${displayTitle}\n`);
        if (contentText) {
    ...
    function clearPrompt() {
        ...
        process.stdout.write(cursor.move(-999, -linesToClear) + erase.down(linesToClear));
    }
    ...
    while (true) {
        ...
        if (isValid) {
            ...
            clearPrompt();
            displayPrompt();
            process.stdout.write(`${erase.line}${currentState.symbol} ${errorMessage}\n`);
        }
    }
}
