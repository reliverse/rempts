...
function toCharCodeStr(charCode) {
    ...
    charCodeStr =
        charCodeStr.length > 1 ? "\\x" + charCodeStr : "\\x0" + charCodeStr;
    return charCodeStr;
}
function toCharOrCharCodeStr(charCode) {
    if (charCode <= 0x1f || charCode === 0x7f) {
        return toCharCodeStr(charCode);
    }
    return String.fromCharCode(charCode);
}
function defaultArgs(args, defaultArgs_) {
    if (!defaultArgs_ || !defaultArgs_.length) {
        return args;
    }
...
SequencesReader.prototype.streamToEvent = async function (stream) {
    var charCode, charCodeStr, char, bytes, codepoint, type, args, argIndex, emitParsedSequence, event, subTree, subSubTree, basePtr, charBuffer = Buffer.alloc(6);
    for (;;) {
        ...
        if (charCode <= 0x1f || charCode === 0x7f) {
            ...
            if (charCode === 0x1b) {
                // Escape, probably an escape sequence!
                charCode = (await readStream(stream))[0];
                if (charCode === 0x5b) {
                    // [ =CSI
                    args = "";
                    ...
                    if (args.length &&
                        CSI_TYPE_EXTRA_CHAR_AFTER.has(args[args.length - 1][args[args.length - 1] - 1])) {
                        ...
                        if (subTree.subTree && args.length) {
                            ...
                            for (argIndex = 0; argIndex < args.length; argIndex++) {
                                ...
                                if (subSubTree) {
                                    ...
                                    if (event !== "none") {
                                        this.emit(event, subSubTree.subType || basePtr.subType || subTree.subType, subSubTree.arg !== undefined
                                            ? subSubTree.arg
                                            : basePtr.arg !== undefined
                                                ? basePtr.arg
                                                : subTree.arg /* eslint-disable-line no-nested-ternary */, subSubTree.extraArgs ||
                                            basePtr.extraArgs ||
                                            subTree.extraArgs ||
                                            defaultArgs(args.slice(argIndex + 1), subSubTree.defaultExtraArgs));
                                    }
                            ...
                            if (subTree.event !== "none") {
                                this.emit(subTree.event, subTree.subType, subTree.arg, subTree.extraArgs ||
                                    defaultArgs(args, subTree.defaultExtraArgs));
                            }
                            emitParsedSequence = false;
                        }
                    }
                    ...
                    for (;;) {
                        ...
                        if (charCode === 0x07) {
                            break;
                        } // This is the OSC terminator, leave
                        if (charCode === 0x1b) {
                            charCode = (await readStream(stream))[0];
                            if (charCode === 0x5c) {
                                break;
                            } // ESC \ =string terminator, so we leave
