...
*/
export default function progressBar_(options) {
    if (!options || typeof options !== "object") {
        options = {};
    }
    var controller = {}, progress, ready = false, pause = false, maxItems, itemsDone = 0, itemsStarted = [], itemFiller, title, titleFiller, width, y, startX, endX, oldWidth, wheel, wheelCounter = 0, itemRollCounter = 0, updateCount = 0, progressUpdateCount = 0, lastUpdateTime, lastRedrawTime, startingTime, redrawTimer, etaStartingTime, lastEta, etaFiller;
    ...
    var etaString = (updated) => {
        var eta = "", elapsedTime, elapsedEtaTime, remainingTime, averageUpdateDelay, averageUpdateProgress, lastUpdateElapsedTime, fakeProgress;
        if (progress >= 1) {
            eta = " done";
        }
        else if (progress > 0) {
            ...
            if (!updated && progressUpdateCount > 1) {
                ...
                if (lastUpdateElapsedTime < averageUpdateDelay) {
                    fakeProgress =
                        progress +
                            (averageUpdateProgress * lastUpdateElapsedTime) /
                                averageUpdateDelay;
                }
                else {
            ...
            remainingTime =
                (elapsedEtaTime * ((1 - fakeProgress) / fakeProgress)) / 1000;
            eta = " in ";
            if (remainingTime < 10) {
    ...
    var redraw = (updated) => {
        ...
        if ((!progress || progress < 1) &&
            lastRedrawTime &&
            time < lastRedrawTime + options.minRefreshTime) {
            if (!options.syncMode) {
                if (redrawTimer) {
        ...
        progressSize =
            progress === undefined
                ? 1
                : Math.round(innerBarSize * Math.max(Math.min(progress, 1), 0));
