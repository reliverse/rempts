...
export const readdir = async (dir, accept = null) => {
    if (dir[dir.length - 1] !== "/") {
        dir += "/";
    }
...
export const statsFilter = (stats, accept) => {
    if ((stats.isFile() && !accept.file) ||
        (stats.isDirectory() && !accept.directory)) {
        return false;
    }
    return true;
};
export const resolveBaseDir = async (baseDir) => {
    if (!baseDir) {
        baseDir = process.cwd();
    }
    else {
...
export const autoCompleteFile = async (inputString, params) => {
    var inputDir, inputFile, currentDir, files, completion, baseDir = params.baseDir;
