import xterm from "../colorScheme/xterm.json" with { type: "json" };
...
handler.mouseX11Protocol = function (basename, buffer) {
    ...
    if (code & 32) {
        if (code & 64) {
            ...
            switch (code & 3) {
                case 0:
                    result.name = basename + "_LEFT_BUTTON_PRESSED";
                    break;
                case 1:
                    result.name = basename + "_MIDDLE_BUTTON_PRESSED";
                    break;
                case 2:
                    result.name = basename + "_RIGHT_BUTTON_PRESSED";
                    break;
                case 3:
                    result.name = basename + "_BUTTON_RELEASED";
                    break;
            }
        }
    }
...
handler.screenSize = function (basename, paramString) {
    var params = paramString.split(";"), width = parseInt(params[1], 10), height = parseInt(params[0], 10), resized = this.root.width !== width || this.root.height !== height;
...
for (let i = 1; i <= 26; i++) {
    ...
    keymap["ALT_" + String.fromCharCode(64 + i)] =
        "\x1b" + String.fromCharCode(96 + i);
    keymap["CTRL_ALT_" + String.fromCharCode(64 + i)] =
        "\x1b" + String.fromCharCode(i);
    keymap["ALT_SHIFT_" + String.fromCharCode(64 + i)] =
