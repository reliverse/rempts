...
export const guessTerminal = function (unpipe) {
    var envVar, version;
    ...
    var tTrueColor = process.env.COLORTERM &&
        process.env.COLORTERM.match(/^(truecolor|24bits?)$/);
    var appId = process.env.COLORTERM && !tTrueColor
    ...
    var safe = appId !== process.env.TERM ||
        (process.env.TERM &&
...
export const getParentTerminalInfo = async function (callback) {
    ...
    try {
        ...
        while (loopAgain) {
            ...
            if (platform === "darwin") {
                appName = path.parse(appName).name;
            }
...
export const getDetectedTerminal = async function (callback) {
    var terminal, info, guessed = termkit.guessTerminal();
