...
// @ts-nocheck
const builtinBarChars = spChars.bar;
function Bar(options) {
    ...
    else if (typeof options.barChars === "string" &&
...
Bar.prototype.preDrawSelf = function () {
    var index, x, fullCells, emptyCells, partialCellRate, fullContent, fullContentWidth = 0, emptyContent, emptyContentWidth = 0, noPartialCell = false, partialCell = null, innerSize = this.outputWidth - 2, rate = (this.value - this.minValue) / (this.maxValue - this.minValue);
    if (!rate || rate < 0) {
        rate = 0;
    }
    else if (rate > 1) {
